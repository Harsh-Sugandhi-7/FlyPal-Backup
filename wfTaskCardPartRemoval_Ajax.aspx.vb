...
Public Class wfTaskCardPartRemoval_Ajax
...
#Region " Business Methods "
    ...
    End Sub
    Private Sub SetPage()
        If Session("PartRemovalEdit") Then
            lblTitle.Text = "Part Removal [" & mTaskCard.TaskCardPartRemovals.CurrentItem.PartNo & " ]"
        Else
            lblTitle.Text = "Part Removal [New]"
        End If
        txtTaskCardNo.Text = mTaskCard.TaskCardNo
    End Sub
    Private Function setObject() As Boolean
        'mTaskCard.TaskCardPartRemovals.CurrentItem.ItemID = New Guid(PartID) 'New Guid(cmbItemList.SelectedValue.ToString)
        ItemID = IIf(txtItemList.Text = "", Guid.Empty, mItemListForCombo(PartNo).ID)
        mTaskCard.TaskCardPartRemovals.CurrentItem.ItemID = ItemID

        mTaskCard.TaskCardPartRemovals.CurrentItem.PartNo = Trim(txtPartNo.Text)
        mTaskCard.TaskCardPartRemovals.CurrentItem.Description = Trim(txtDescription.Text)
        mTaskCard.TaskCardPartRemovals.CurrentItem.RequiredQty = Val(txtReqQty.Text)
        mTaskCard.TaskCardPartRemovals.CurrentItem.Remark = Trim(txtRemark.Text)
        'Added By Shweta On 5-Sep-2013 For BA04092013
        mTaskCard.TaskCardPartRemovals.CurrentItem.OffSerialNo = Trim(txtOffSerialNo.Text)
        mTaskCard.TaskCardPartRemovals.CurrentItem.OnSerialNo = Trim(txtOnSerialNo.Text)
        'End
        mTaskCard.TaskCardPartRemovals.CurrentItem.IsPartRemoval = True
        mTaskCard.TaskCardPartRemovals.CurrentItem.Position = Trim(txtPosition.text)
        Return True
    End Function
    ...
    Private Sub SaveTaskCard()
        Try
            mTaskCard.Save()
            MarkLog(Util.Action.Save, "TaskCard", "Task Card Part Removal : " + Chr(13) + "Part No. : " + txtPartNo.Text + "," + "Description : " + txtDescription.Text + "," + "Qty. : " + txtReqQty.Text, Util.ErrorType.NoError, mTaskCard.ID, EventLogID)
...
#Region " Data Binding "
    Private Sub DataFieldBind()
        ...
        DataBind()
        txtItemList.Text = IIf(Not mTaskCard.TaskCardPartRemovals.CurrentItem.ItemID.Equals(Guid.Empty), mTaskCard.TaskCardPartRemovals.CurrentItem.PartNo + " [" + mTaskCard.TaskCardPartRemovals.CurrentItem.Description + "]", "")
        hdnpartId.Value = IIf(Not mTaskCard.TaskCardPartRemovals.CurrentItem.ItemID.Equals(Guid.Empty), mTaskCard.TaskCardPartRemovals.CurrentItem.ItemID.ToString, "{00000000-0000-0000-0000-000000000000}")
    End Sub
    ...
    Private Function CustomValidate1() As Boolean
        ...
        ''    For i As Integer = 0 To mTaskCard.GetBrokenRulesCollection.Count - 1
        ''        strMSG = strMSG + mTaskCard.GetBrokenRulesCollection(i).Description + "<Br>"
        ''    Next
        ''End If
        Dim mTaskCardSpare As TaskCardSpare
        If Not mTaskCard.TaskCardPartRemovals.IsValid Then
            For Each mTaskCardSpare In mTaskCard.TaskCardPartRemovals
                For i As Integer = 0 To mTaskCardSpare.GetBrokenRulesCollection.Count - 1
                    strMSG = strMSG + mTaskCardSpare.GetBrokenRulesCollection(i).Description + "<Br>"
                Next
            Next
        End If
        If strMSG.Trim <> "" Then
...
#Region " Events "
    ...
    Private Sub btnOK_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnOK.Click
        ...
        SetPartID()
        If Session("PartRemovalEdit") = False Then
            If Trim(txtPartNo.Text) <> "" And Not mTaskCard.TaskCardPartRemovals.Contains(mTaskCard.TaskCardPartRemovals.CurrentItem.ID, mTaskCard.ID, Trim(txtPartNo.Text)) Then
                setObject()
            ...
            ElseIf Trim(txtPartNo.Text) <> "" And mTaskCard.TaskCardPartRemovals.Contains(mTaskCard.TaskCardPartRemovals.CurrentItem.ID, mTaskCard.ID, Trim(txtPartNo.Text)) Then
                MSGBoxCtrl.show(MSGBox.Message_title.Duplicate, MSGBox.Message_text.Duplicate, "", MsgBoxStyle.OkOnly, "")
                mTaskCard.TaskCardPartRemovals.CurrentItem.Description = ""
                Exit Sub
            Else
                setObject()
                If Not CustomValidate1() Then
        ...
        Else
            ...
            If mTaskCard.TaskCardPartRemovals.CurrentItem.IsDirty Then
                'If Not mTaskCard.TaskCardPartRemovals.Contains(mTaskCard.TaskCardPartRemovals.CurrentItem.ID, mTaskCard.TaskCardPartRemovals.CurrentItem.TaskCardID, New Guid(PartID)) Then
                ...
                If EitherPartNoOrItemID = True Then
                    ...
                    'ItemID = IIf(txtItemList.Text = "", Guid.Empty, mItemListForCombo(PartNo).ID)
                    'If Not mTaskCard.TaskCardPartRemovals.Contains(mTaskCard.TaskCardPartRemovals.CurrentItem.ID, mTaskCard.TaskCardPartRemovals.CurrentItem.TaskCardID, ItemID) Then
                    If Not CustomValidate1() Then
                        mTaskCard = clnTaskCard
                        Session("mTaskCard") = mTaskCard
                        Session("wfTaskCard.TaskCard") = clnTaskCard
                ...
                ElseIf Trim(txtPartNo.Text) <> "" And mTaskCard.TaskCardPartRemovals.Contains(mTaskCard.TaskCardPartRemovals.CurrentItem.ID, mTaskCard.TaskCardPartRemovals.CurrentItem.TaskCardID, Trim(txtPartNo.Text)) Then   'ElseIf Not mTaskCard.TaskCardPartRemovals.Contains(New Guid(PartID)) Then
                    If Not CustomValidate1() Then
                        mTaskCard = clnTaskCard
                        Session("mTaskCard") = mTaskCard
                        Session("wfTaskCard.TaskCard") = clnTaskCard
    ...
    Private Sub txtItemList_TextChanged(sender As Object, e As System.EventArgs) Handles txtItemList.TextChanged
        'DataFieldBind()
        'txtPartNo.Text = IIf(txtItemList.SelectedIndex > 0, mItemListForCombo(txtItemList.SelectedIndex).Name, "")
        'txtDescription.Text = IIf(txtItemList.SelectedIndex > 0, mItemListForCombo(txtItemList.SelectedIndex).Description, "")
        'ControlVisibility()
    ...
    Private Sub btnClose_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnClose.Click
        If mTaskCard.TaskCardPartRemovals.CurrentItem.IsNew And Not Session("PartRemovalEdit") = True Then mTaskCard.TaskCardPartRemovals.Remove(mTaskCard.TaskCardPartRemovals.CurrentItem)
        Session("PartRemovalEdit") = False
        upnlValidationsummary.Update()
        Session.Remove("PartID")
        Session.Remove("mItemListForCombo")
