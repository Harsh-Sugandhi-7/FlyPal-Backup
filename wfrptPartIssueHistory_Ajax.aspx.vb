Public Class wfrptPartIssueHistory_Ajax
...
#Region " Variable Declaration "
    ...
    Dim mPartIssueHistorySearchingCriteria As String = String.Empty
...
#Region " Helper Methods "
    ...
    Private Sub SetValues()
        ...
        mPartIssueHistorySearchingCriteria = lblPartNo.Text + ", " + lblDesc.Text.Trim
    End Sub
    Private Sub SetReport(ByVal IsExcel As Boolean)
        ...
        myReport = New crptIssItemHistoryNew
        rpt = rptPartHistory.GetPartHistory(PartNo, Description, 3, "", chkIsValued.Checked)
        ...
        If rpt.Count <= 0 Then
            ...
            Exit Sub
        Else
            RecentMenuEvent.RecentMenuItemEvent(User.Identity.Name, 705)
        End If
        ds.Clear()
        ...
        MarkLog(Util.Action.Print, "PartIssueHistory", mPartIssueHistorySearchingCriteria, Util.ErrorType.NoError, Guid.Empty, EventLogID)
    End Sub
...
#Region " Events "
    ...
    Private Sub btnExport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExport.Click
        If IsValid Then
            ...
            da.Fill(ds, "ExcelIssuerptPartHistory", rpt)
            da.Fill(ds, "rptSearchingCriteria", objsearch)
            Dim columnToRemove1 As String() = {"Remark", "PartID", "PartName", "PartDescription", "OrderFor", "Type", "ReleaseNoteDate", "CureDate", "ExpiryDate", "RecQty", "OrdQty", "IssueTo", "StartDate", "VendorInvoiceNo", "VendorInvoiceDate", "VendorInvoiceDetail", "CureQtrs", "CureYear", "ExpQtrs", "ExpYear", "CureQtrYear", "ExpQtrYear", "BatchNo", "Location", "TransType", "AlternateParts", "TransTypeName", "EffRate", "ATAChapter", "PartStatus", "OrderNumber", "OrderDateFormatted", "OrderType", "TransTypeID", "Applicability", "ATACode", "ATANomenclature", "OrderText", "OrderNo", "OrderAmend", "IsOverhaul", "OrderTransTypeID", "OrderDate"}
            For i As Integer = 0 To columnToRemove1.Length - 1
                If ds.Tables("ExcelIssuerptPartHistory").Columns.Contains(columnToRemove1(i)) Then
                    ds.Tables("ExcelIssuerptPartHistory").Columns.Remove(columnToRemove1(i))
                End If
            Next
            ...
            If ds.Tables("ExcelIssuerptPartHistory").Columns.Contains("PHDate") Then
                ds.Tables("ExcelIssuerptPartHistory").Columns("PHDate").ColumnName = "Date "
            End If

            If ds.Tables("ExcelIssuerptPartHistory").Columns.Contains("IdentityNo") Then
                ds.Tables("ExcelIssuerptPartHistory").Columns("IdentityNo").ColumnName = "Issue No."
            End If
            If ds.Tables("ExcelIssuerptPartHistory").Columns.Contains("ToFrom") Then
                ds.Tables("ExcelIssuerptPartHistory").Columns("ToFrom").ColumnName = "Issue To"
            End If
            If ds.Tables("ExcelIssuerptPartHistory").Columns.Contains("ReleaseNoteNo") Then
                ds.Tables("ExcelIssuerptPartHistory").Columns("ReleaseNoteNo").ColumnName = "Rel. Note No."
            End If

            If ds.Tables("ExcelIssuerptPartHistory").Columns.Contains("SerialNo") Then
                ds.Tables("ExcelIssuerptPartHistory").Columns("SerialNo").ColumnName = "Serial No."
            End If
            If ds.Tables("ExcelIssuerptPartHistory").Columns.Contains("IssQty") Then
                ds.Tables("ExcelIssuerptPartHistory").Columns("IssQty").ColumnName = "Iss Qty."
            End If
            ...
            dsNew.Merge(ds.Tables("ExcelIssuerptPartHistory"))

            dsNew.Tables("rptSearchingCriteria").TableName = "Searching Criteria"
            dsNew.Tables("ExcelIssuerptPartHistory").TableName = "Part Issue History"
			Session("ExcelFileName") = "Part Issue History"
			Session("dsNew") = dsNew
            ...
            MarkLog(Util.Action.Print, "PartIssueHistory", "Export To excel " + mPartIssueHistorySearchingCriteria, Util.ErrorType.NoError, Guid.Empty, EventLogID) 'Added by Shital on 18-Jan-2021
            'If Page.IsValid Then
            '    SetValues()
            '    SetReport(True)
        Else
