...
Public Class wfnWOQCApprovalList
...
#Region " Variable Declaration "
	Public mnWO As nWO
	Public mWOQCApprovalList As nWOList
	...
	Dim SearchIndex, DateIndex, FromDate, ToDate, WOText, StatusID, No, WOStatusID, RegNo, ModelName, QCStatus As String
...
#Region " Business Methods "
	...
	Private Sub GetSession()
		...
		mWOQCApprovalList = Session("mWOQCApprovalList")
	...
	Private Sub SetSession()
		...
		Session("mWOQCApprovalList") = mWOQCApprovalList
	...
	Private Sub RemoveSession()
		...
		Session.Remove("mWOQCApprovalList")
	...
	Private Sub ClearAll()
		...
		'  If InStr(Session("MiddleFrame"), "wfnWOQCApprovalList.aspx?TransTypeID=" & Request.QueryString("TransTypeId")) <= 0 Then
		If InStr(Session("MiddleFrame"), "wfnWOQCApprovalList.aspx?") <= 0 Then
			RemoveSession()
			Session.Remove("mWOQCApprovalList")
	...
	Private Sub SetTitle()
		'Dim mWOQCApprovalList As nWOList
		'mWOQCApprovalList = nWOList.GetWOList()
		...
		mWOQCApprovalList = Session("mWOQCApprovalList")
		totcnt = mWOQCApprovalList.TotalWOCount
		Session("totcnt") = totcnt
		lblTitle.Text = "List for QC Approval Work Order" ' [Total No of Record(s):-" + totcnt.ToString() + "]"
	End Sub
	...
	Private Sub setVariables()
		...
		No = txtNo.Text.Trim
		QCStatus = IIf(cmbQcStatus.SelectedIndex < 0, 0, cmbQcStatus.SelectedValue)
	...
	Private Sub SetControl()
		...
		txtNo.Text = No
		cmbQcStatus.SelectedValue = IIf(QCStatus Is Nothing, "3", QCStatus)
		...
		If (AppSettings("ClientCode") IsNot Nothing) AndAlso (AppSettings("ClientCode") = "TAAL" Or AppSettings("ClientCode") = "GlobalJet") Then
			...
			dgWOList.DataBind()
			lblResult.Text = "List of Engineering Order for QC Approval as per criteria :" & mWOQCApprovalList.Count & " Record(s) found."
		Else
			dgWOList.Columns(2).HeaderText = "W.O. No."

			dgWOList.DataBind()
			lblResult.Text = "List of Work Order for QC Approval as per criteria :" & mWOQCApprovalList.Count & " Record(s) found."
		End If
		GridColumnsVisibility()
	End Sub
	...
	Private Sub FindNow(Optional ByVal Text As String = "", Optional ByVal No As Int32 = 0, Optional ByVal FromDate As String = "", Optional ByVal ToDate As String = "", Optional ByVal RegNo As String = "", Optional ByVal ModelName As String = "", Optional ByVal WOStatusID As Integer = 0, Optional ByVal StatusID As Integer = 0, Optional ByVal AddTopItem As String = "")
		...
		mWOQCApprovalList = Nothing
		dgWOList.DataSource = Nothing

		mWOQCApprovalList = nWOList.GetWOList(Text, No, FromDate, ToDate, RegNo, ModelName, StatusID, WOStatusID, AddTopItem, , mTransTypeID, IsForQC:=IIf(IsAllWOsTicked = False, True, False))
		dgWOList.DataSource = mWOQCApprovalList
		dgWOList.DataBind()
		upnlGridView.Update()
		Session("mWOQCApprovalList") = mWOQCApprovalList
		totcnt = mWOQCApprovalList.TotalWOCount
	...
	Private Sub CallFindNow(ByVal Index As Integer)
		FindNow(WOText, CInt(Val(No)), txtFromDate.Text.ToString, txtToDate.Text.ToString, RegNo:=RegNo, ModelName:=ModelName, WOStatusID:=CInt(Val(QCStatus)), StatusID:=2)
	End Sub
				...
				Case MsgBoxResult.Yes
					If MSGBoxCtrl.Sender = "Delete" Then
						...
						Try
							...
							TempWOID = mnWO.ID
							mWOQCApprovalDetail = mnWO.WONumber + " Dated : " + mnWO.WODateFormatted + " Created By : " + mnWO.WOBy + IIf(Not mnWO.MachineID.Equals(Guid.Empty), " Aircraft : " + mnWO.RegNo, "") + IIf(mnWO.ModelName <> "", " Model : " + mnWO.ModelName, "") + IIf(mnWO.SerialNo <> "", " Serial No. : " + mnWO.SerialNo, "")
							MarkLog(Util.Action.Delete, "Work Order", mWOQCApprovalDetail, Util.ErrorType.NoError, TempWOID, EventLogID)
						...
						Catch ex As SqlException
							...
							ElseIf ex.Number = 547 Then
								MarkLog(Util.Action.Delete, "Work Order", "Can't delete : " & mWOQCApprovalDetail & " is Currently in use", Util.ErrorType.NoError, TempWOID, EventLogID)
...
#Region "DataFieldBind"
	Private Sub DataFieldBind()
		...
		RegNo = Session("RegNo")
		ModelName = Session("ModelName")
		QCStatus = IIf(QCStatus Is Nothing, 3, QCStatus)
		IsAllWOsTicked = Session("IsAllWOsTicked")  'Added by Saylee on 14-Jan-2020
	...
	Private Sub SetGrid()
		...
		For j As Integer = 0 To dgWOList.Rows.Count - 1
			...
			If Me.dgWOList.Rows.Item(j).Cells(11).Text = Trans.WOCAMO Then
				If Me.dgWOList.Rows.Item(j).Cells(3).Text = "" Then '    If mMachineNameValueList(Me.dgWOList.Rows.Item(j).Cells(3).Text) Is Nothing Then
					IsReadOnly = True
				ElseIf Me.dgWOList.Rows.Item(j).Cells(3).Text = "&nbsp;" Or mMachineNameValueList(Me.dgWOList.Rows.Item(j).Cells(3).Text) Is Nothing Then
					IsReadOnly = False
				Else
				...
				End If
			ElseIf Me.dgWOList.Rows.Item(j).Cells(11).Text = Trans.WO145 Then
				IsReadOnly = False
			End If

			If IsReadOnly = True Then
				dgWOList.Rows(j).Cells(10).Enabled = False
			Else
				dgWOList.Rows(j).Cells(10).Enabled = True
			End If
			...
			If dgWOList.Rows.Item(j).Cells(8).Text = "QC Rejected" Then
				Me.dgWOList.Rows.Item(j).Cells(8).ForeColor = Color.Red
				Me.dgWOList.Rows.Item(j).Cells(8).Font.Bold = True
			ElseIf dgWOList.Rows.Item(j).Cells(8).Text = "QC Approved" Then
				Me.dgWOList.Rows.Item(j).Cells(8).ForeColor = Color.Green
				Me.dgWOList.Rows.Item(j).Cells(8).Font.Bold = True
			ElseIf dgWOList.Rows.Item(j).Cells(8).Text = "None" Then
				Me.dgWOList.Rows.Item(j).Cells(8).ForeColor = Color.Violet
				Me.dgWOList.Rows.Item(j).Cells(8).Font.Bold = True
			End If
...
#Region "Events"
	Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
		...
		If Not IsPostBack Then
			...
			'End If
			Session("MiddleFrame") = "wfnWOQCApprovalList.aspx?"
			DataFieldBind()
			SetControl()
		Else
			dgWOList.DataSource = mWOQCApprovalList
	...
	Private Sub dgWOList_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles dgWOList.RowCommand
		Select Case e.CommandName
			Case "EditRec"
				...
				'   Dim mId As Guid = mWOQCApprovalList(Index).ID
				Dim mID As Guid = New Guid(e.CommandArgument.ToString)
				Dim mDate As String = mWOQCApprovalList(mID).WODateFormatted
				Dim mWorkOrderNo As String = mWOQCApprovalList(mID).WONumber
				Dim mCreatedBy As String = mWOQCApprovalList(mID).WOBy
				Dim mRegNo As String = IIf(mWOQCApprovalList(mID).RegNo = "", "", mWOQCApprovalList(mID).RegNo)
				Dim mModel As String = IIf(mWOQCApprovalList(mID).ModelName = "", "", mWOQCApprovalList(mID).ModelName)
				Dim mSerialNo As String = mWOQCApprovalList(mID).SerialNo
				mWOQCApprovalDetail = mWorkOrderNo + " Dated : " + mDate + " Created By : " + mCreatedBy + IIf(mRegNo <> "", " Aircraft : " + mRegNo, "") + IIf(mModel <> "", " Model : " + mModel, "") + IIf(mSerialNo <> "", " Serial No. : " + mSerialNo, "")
				MarkLog(Util.Action.Edit, "Work Order", mWOQCApprovalDetail, Util.ErrorType.NoError, mId, EventLogID)
				EditRecord(mId)
				Session("Edit") = True
				DataFieldBind()
				SetControl()
				...
				If chkShowAllWOs.Checked And Not (cmbStatus.SelectedValue = 5 Or cmbStatus.SelectedIndex = 6) Then  'Added by Saylee on 14-Jan-2020
				...
				Else
					...
					If mFileAttach.Size > 0 Then
						...
						If File.Exists(AppSettings("DOCPath")) = False Then
							...
							MarkLog(Util.Action.View, "Work Order", Detail, Util.ErrorType.HandledError, mWOQCApprovalList(Idx).ID, EventLogID)
						End If
					End If
				End If
		End Select
	...
	Private Sub btnFindNow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnFindNow.Click
		...
		'lblResult.Text = "List of Work Order as per criteria :" & mWOQCApprovalList.Count & " Record(s) found"
	End Sub
	...
	Private Sub dgWOList_Sorting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewSortEventArgs) Handles dgWOList.Sorting
		mWOQCApprovalList.Sort(e.SortExpression, ComponentModel.ListSortDirection.Ascending)
		dgWOList.DataSource = mWOQCApprovalList
		Session("mWOQCApprovalList") = mWOQCApprovalList
		dgWOList.DataBind()
		SetGrid()
	...
	End Sub
	Private Sub cmbQcStatus_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles cmbQcStatus.SelectedIndexChanged
		If cmbQcStatus.SelectedValue = 0 Then
			FindNow(WOText, CInt(Val(No)), txtFromDate.Text.ToString, txtToDate.Text.ToString, RegNo:=RegNo, ModelName:=ModelName, WOStatusID:=3, StatusID:=2)
		ElseIf cmbQcStatus.SelectedValue = 1 Then
			FindNow(WOText, CInt(Val(No)), txtFromDate.Text.ToString, txtToDate.Text.ToString, RegNo:=RegNo, ModelName:=ModelName, WOStatusID:=5, StatusID:=2)
		ElseIf cmbQcStatus.SelectedValue = 2 Then
			FindNow(WOText, CInt(Val(No)), txtFromDate.Text.ToString, txtToDate.Text.ToString, RegNo:=RegNo, ModelName:=ModelName, WOStatusID:=6, StatusID:=2)
		End If
