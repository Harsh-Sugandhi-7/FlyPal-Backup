...
Public Class wfMachineCertificateRenewList_AJAX
...
#Region " Variable Declaration "
    ...
    Dim mSortedMachineCertificateList As List(Of MachineCertificateList.MachineCertificateListInfo) = New List(Of MachineCertificateList.MachineCertificateListInfo)
...
#Region " Business Methods "
    Private Sub GetSession()
        ...
        mRenewMachineCertificateList = CType(Session("mRenewMachineCertificateList"), MachineCertificateList)
            ...
            Select Case Result1
                Case MsgBoxResult.Yes
                    If MSGBoxCtrl.Sender = "Delete" Then
                        ...
                        Finally
                            If msgCount = 0 Then
                                'MarkLog(Util.Action.Delete, "RenewalCertificate", mRenewMachineCertificateList.Item(mRenewMachineCertificateList.CurrentIndex).CertificateName, Util.ErrorType.NoError, mRenewMachineCertificateList.Item(mRenewMachineCertificateList.CurrentIndex).ID)
                            End If
                        End Try
                    End If
    ...
    Private Sub ControlVisibility()
        btnPrint.Enabled = (mRenewMachineCertificateList.Count > 0)
        'btnPrintTop.Enabled = (mRenewMachineCertificateList.Count > 0)

        'If chkApplicable.Checked = True Then
        '    dgCertificateList.Columns(13).Visible = False
        '    dgCertificateList.Columns(15).Visible = False
        'Else
        '    dgCertificateList.Columns(13).Visible = True
        '    dgCertificateList.Columns(15).Visible = True
        'End If
    End Sub
    Private Sub FindNow()
        ...
        mRenewMachineCertificateList = MachineCertificateList.GetMachineCertificateList(New Guid(cmbAircraftList.SelectedValue.ToString), Today.Date.ToString, chkApplicable.Checked, CertificateName:=Trim(txtCertificateName.Text))

        mSortedMachineCertificateList = (From c As MachineCertificateList.MachineCertificateListInfo In mRenewMachineCertificateList
    ...
    Private Sub SetPage()
        ...
        lblResult.Text = "List of Certificates as per selected criteria : " & mRenewMachineCertificateList.Count & " Record(s) found."
    End Sub
    Private Sub ClearAll()
        If Session("MiddleFrame") <> "wfMachineCertificateRenewList_Ajax.aspx?" Then
    ...
    Private Sub RenewRecord(ByVal Index As Int32, ByVal ID As Guid)
        mMachineCertificate = MachineCertificate.GetMachineCertificate(mRenewMachineCertificateList(Index).MachineID, ID)
        mRenewMachineCertificate = MachineCertificate.NewChildRenewalMachineCertificate(mRenewMachineCertificateList(Index).MachineID, _
        ...
        '**************************************************
        Session("RegNo") = mRenewMachineCertificateList(Index).RegNo 'cmbAircraftList.SelectedItem.Text
        'Dim str As String
    ...
    Private Sub HistoryRecord(ByVal Index As Int32, ByVal ID As Guid, MachineID As Guid)
        ...
        Session("RegNo") = mRenewMachineCertificateList(Index).RegNo
    ...
    Private Sub EditRecord(ByVal Index As Int32, ByVal ID As Guid)
        ...
        mRenewMachineCertificate = MachineCertificate.GetRenewalMachineCertificate(mRenewMachineCertificateList(Index).MachineID, ID)
        If mRenewMachineCertificate.IsMaster Then
            'MessageBox.Show("This is a master record and can not be edited from here", "Comply Component Monitor Inspection Status", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1)
        ...
        Else
            ...
            '**************************************************
            'MarkLog(Util.Action.Edit, "RenewalCertificate", " Certificate -> " + mMachineCertificate.CertificateName, Util.ErrorType.NoError, mMachineCertificate.ID)
            mMachineCertificateDetails = "Reg No. : " + mRenewMachineCertificateList(Index).RegNo & " Name : " & mMachineCertificate.CertificateName & " No.: " & mMachineCertificate.CertificateNo
    ...
    Private Sub DeleteRecord(ByVal Index As Int32, ByVal ID As Guid)
        ...
        Else
            ...
            mMachineCertificate = MachineCertificate.GetMachineCertificate(mRenewMachineCertificate.MachineID, mRenewMachineCertificate.ReferenceID)
            mRenewMachineCertificateList.CurrentIndex = Index
            Session("mRenewMachineCertificateList") = mRenewMachineCertificateList
            Session("mRenewMachineCertificate") = mRenewMachineCertificate
            Session("mMachineCertificate") = mMachineCertificate
    ...
    Private Sub SetGrid()
        ...
        '    'View Link
        '    If Me.dgCertificateList.Rows.Item(j).Cells(18).Text = "" Then
        '        P = 0
        '    Else
        '        P = CType(Me.dgCertificateList.Rows.Item(j).Cells(18).Text, Integer)
        '    End If
        '    If P <= 0 Or IsReadOnly = True Then
        '        Me.dgCertificateList.Rows.Item(j).Cells(16).Enabled = False
        '    End If

        '    'History Link
        '    If Me.dgCertificateList.Rows.Item(j).Cells(19).Text = "" Then
        '        B = False
        '    Else
        '        B = CType(Me.dgCertificateList.Rows.Item(j).Cells(19).Text, Boolean)
        '    End If
        '    If B = True Or IsReadOnly = True Then
        '        Me.dgCertificateList.Rows.Item(j).Cells(17).Enabled = False
        '    End If

        '    'Renew Link
        '    'Disable Link if One Time certificate
        '    Dim chkBox As CheckBox
        '    chkBox = CType(Me.dgCertificateList.Rows.Item(j).FindControl("chkIsOneTimeCertificate"), CheckBox)
        '    If chkBox.Checked = True Or IsReadOnly = True Then
        '        Me.dgCertificateList.Rows.Item(j).Cells(13).Enabled = False
        '    End If

        '    Me.dgCertificateList.Rows.Item(j).Cells(14).Enabled = Not (IsReadOnly = True) 'Edit link
        '    Me.dgCertificateList.Rows.Item(j).Cells(15).Enabled = Not (IsReadOnly = True) 'Delete link
        ...
        If IsReadOnly = True Then
            btnByMail.Enabled = False
            lblReadOnly.Visible = True
        Else
            btnByMail.Enabled = True
    ...
    Private Sub SetReport(Optional byMail As Boolean = False)
        If Not User.IsInRole("RenewalCertificatePrint") Then
            ...
            Exit Sub
        End If
        Dim Rpt As New crMachineRenewCertificates
        Dim da As New CSLA.Data.ObjectAdapter
        Dim ds As New dsMachineCertificatesForDueReport
        Dim rptMachineCertificateList As New MachineCertificateList
        Dim mCompanyDetail As New CompanyDetail

        rptMachineCertificateList = MachineCertificateList.GetMachineCertificateList(New Guid(cmbAircraftList.SelectedValue.ToString), Today.Date.ToString, chkApplicable.Checked, CertificateName:=Trim(txtCertificateName.Text))
        mMachineNameValueList = Session("mMachineNameValueList")
   ...
   mCompanyDetail.WebSite, "Aircraft Certificate Report", AircraftName, Trim(txtCertificateName.Text), "", "", "", AppSettings("Product Version"), AppSettings("SINote"), "", "", "", AppSettings("Logo"))
        ...
        da.Fill(ds, rptMachineCertificateList)
        da.Fill(ds, Report)
...
#Region " Data Binding "
    Private Sub DataFieldBind()
        ...
        mRenewMachineCertificateList = MachineCertificateList.GetMachineCertificateList(mMachineId, Today.Date.ToString, chkApplicable.Checked, CertificateName:=CertificateName)
...
#Region " Events "
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not IsPostBack And Session("sender") = "" Then
            ...
            Session("MiddleFrame") = "wfMachineCertificateRenewList_Ajax.aspx?"
            DataFieldBind()
        End If
        SetGrid()
        ControlVisibility()
    ...
    Private Sub dgCertificateList_RowCommand(sender As Object, e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles dgCertificateList.RowCommand
        ...
        Select Case e.CommandName
            Case "RenewRec"
                ...
                'Dim mID As Guid = mRenewMachineCertificateList(index).ID
                Dim mID As Guid = New Guid(dgCertificateList.DataKeys(Index).Values("ID").ToString)
                Dim mName As String = mRenewMachineCertificateList(mID).CertificateName
                Dim mNo As String = mRenewMachineCertificateList(mID).CertificateNo
                mMachineCertificateDetails = "Reg No. : " + cmbAircraftList.SelectedItem.Text & " Name : " & mName & " No.: " & mNo
            ...
            Case "ViewRec"
                ...
                mRenewMachineCertificate = MachineCertificate.GetRenewalMachineCertificate(mRenewMachineCertificateList(Index).MachineID, mID)
            ...
            Case "HistoryRec"
                ...
                Dim mName As String = mRenewMachineCertificateList(mID).CertificateName
                Dim mNo As String = mRenewMachineCertificateList(mID).CertificateNo
                mMachineCertificateDetails = "Reg No. : " + mRenewMachineCertificateList(Index).RegNo & " Name : " & mName & " No.: " & mNo

                HistoryRecord(Index, mID, mRenewMachineCertificateList(Index).MachineID)
        End Select
    End Sub
    Private Sub cmbAircraftList_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles cmbAircraftList.SelectedIndexChanged
        ...
        upnlResult.Update()
    End Sub
    Private Sub chkApplicable_CheckedChanged(sender As Object, e As System.EventArgs) Handles chkApplicable.CheckedChanged, txtCertificateName.TextChanged
        FindNow()
        SetPage()
    ...
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        '     rptMachineCertificateList = MachineCertificateList.GetMachineCertificateList(New Guid(cmbAircraftList.SelectedValue.ToString), Today.Date.ToString, chkApplicable.Checked)
        '     mMachineNameValueList = Session("mMachineNameValueList")
        '     Dim AircraftName As String = cmbAircraftList.SelectedItem.Text 'mMachineList(New Guid(cmbAircraftList.SelectedValue.ToString)).RegNo
        ...
        '     Dim Report As New ReportData(mCompanyDetail.CompanyName, mCompanyDetail.Address, _
        'mCompanyDetail.Tel1, mCompanyDetail.Tel2, mCompanyDetail.Fax, mCompanyDetail.Email, _
        'mCompanyDetail.WebSite, "Aircraft Certificate Report", AircraftName, "", "", "", "", AppSettings("Product Version"), AppSettings("SINote"), "", "", "", AppSettings("Logo"))
        ...
        '         Exit Sub
        '     Else
        '         RecentMenuEvent.RecentMenuItemEvent(User.Identity.Name, 1114)
        '     End If
        ...
        '     Rpt.SetDataSource(ds)
        '     Session("CrystalReport") = Rpt
        '     'MarkLog(Util.Action.Print, "wfMachineCertificateRenewList", "List of Aircraft Certificate Report", Util.ErrorType.NoError, Guid.Empty)
