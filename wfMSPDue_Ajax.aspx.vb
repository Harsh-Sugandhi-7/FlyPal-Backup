...
#Region " Helper Methods "
    ...
    Private Sub SetReport(Optional ByVal IsExcel As Boolean = False)
        ...
        Dim rpt As MSPDue
        Dim ds As New dsMSPDue
        myReport = New crptMSPDue
        rpt = MSPDue.GetMSPDue(AsOnDate:=txtDate.Text, MSPText:=IIf(cmbMSPText.SelectedIndex = 0, "", cmbMSPText.SelectedItem.Text),
                               MSPNo:=Val(txtNo.Text),
                               AssemblyID:=IIf(cmbAssemblyList.SelectedIndex = 0, "{00000000-0000-0000-0000-000000000000}", cmbAssemblyList.SelectedValue),
                               Range:=-1)
        ...
        Dim mReport As New ReportData(mCompanyDetail.CompanyName, mCompanyDetail.Address, mCompanyDetail.Tel1, mCompanyDetail.Tel2, mCompanyDetail.Fax,
                                          mCompanyDetail.Email, WebSite:="", ReportName:="", SearchStr1:=New SmartDate(txtDate.Text).FormattedText, SearchStr2:=IIf(cmbAssemblyList.SelectedIndex = 0, "", cmbAssemblyList.SelectedItem.Text), SearchStr3:=IIf(cmbMSPText.SelectedIndex = 0, "", cmbMSPText.SelectedItem.Text + IIf(txtNo.Text = "", "", "-" + txtNo.Text)),
        ...
        Else
            ...
            If ds.Tables("MSPDue").Columns.Contains("AssemblyName") Then
                ds.Tables("MSPDue").Columns("AssemblyName").ColumnName = "Assembly"
            End If
            ...
            Dim MSPColumnsForExportToExcel As New List(Of String)
            MSPColumnsForExportToExcel.AddRange(New String() {"Remaining Days"})
            Session("MSPColumns") = MSPColumnsForExportToExcel
			Session("ExcelFileName") = "MSP Due"
			Session("dsNew") = dsNew
			ScriptManager.RegisterStartupScript(Me, Me.GetType(), "openFile", "openFile();", True)
            MarkLog(Util.Action.Print, "MSPDue", "Export To Excel ", Util.ErrorType.NoError, Guid.Empty, EventLogID)
        End If
        MarkLog(Util.Action.Print, "MSPDue", "", Util.ErrorType.NoError, Guid.Empty, EventLogID)
    End Sub
...
#Region " Data Binding "
    Private Sub DataFieldBind()
        cmbMSPText.DataSource = DistinctTextListForMSP.GetDistinctTextList("32", , True, "(ALL)")
        cmbAssemblyList.DataSource = AssemblyList.GetAssemblyListForComboBox(0, Guid.Empty.ToString, Today.Date.ToString, "(ALL)", IsInstalled:=True, IsForSpareAssembly:=False)
        DataBind()
    End Sub
#End Region
