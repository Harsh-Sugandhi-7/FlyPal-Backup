'Created By Saylee on 29-Apr-2024

Public Class wfnWOJobCompRemInst
...
#Region " Variable Declaration"
	...
	Dim IsValidRemoval As Boolean = True 'Change
	Dim IsValidInstallation As Boolean = True 'Change
	Dim IsExpiryServicePresent As Boolean = False
	Public mnWOJobComp As nWOJobComp
	Public mnWO As nWO
#End Region
...
#Region " Business Methods "
	Private Sub GetSession()
		...
		mnWOJobComp = Session("mnWOJobComp")
		mnWO = Session("mnWO")
	...
	Private Sub SaveComplyRecord()
		...
		If mCompMonitorServiceStatus IsNot Nothing Then
	...
	Private Sub SetInstObject()
		With mInstCompStatus
			...
			'mAssemblyStatus = AssemblyStatus.GetAssemblyStatus(New Guid(cmbInstAssemblyList.SelectedValue.ToString))
			'.AssemblyID = mAssemblyStatus.AssemblyID  'New Guid(cmbInstAssemblyList.SelectedValue.ToString)
			...
			If cmbInstPartNo.SelectedIndex > 0 Then
				.Comp.PartID = New Guid(cmbInstPartNo.SelectedValue.ToString)
			Else
				If cmbInstPartNo.SelectedIndex = 0 And txtInstPartNo.Text <> "" Then
					Dim obj As Part, objList As PartList = PartList.GetPartList()
					If objList.Contains(Trim(txtInstPartNo.Text.Trim)) = True Then
						obj = Part.GetPart(objList.Item(Trim(txtInstPartNo.Text.Trim)).ID)
						mInstCompStatus.Comp.PartID = obj.ID
					Else
						obj = Part.NewPart(Guid.NewGuid)
						obj.Name = txtInstPartNo.Text.Trim
						obj.Description = IIf(txtInstDescription.Text = "", txtInstPartNo.Text.Trim, txtInstDescription.Text)
						If obj.IsValid Then
							obj.Save()
							mInstCompStatus.Comp.PartID = obj.ID
						End If
	...
	Private Sub SetInstPeroids()
		...
		If mPeriodListForCompStatus IsNot Nothing Then
	...
	Private Sub NewRecord(ByVal LogID As Guid, ByVal mCurrentDate As String)
		...
		If mnWOJobComp.IsForRemoval Then
			mRemovedCompStatus = CompStatus.GetCompStatus(mRemCompStatus.ID, mRemAssemblyStatus.ID, mCurrentDate)
			...
			If mnWOJobComp.IsForInstall = True Then
				mInstCompStatus.InstallationWONo = mPrevCompStatus.RemovalWONo
		...
		Else
			mInstCompStatus = CompStatus.NewInstallCompStatus(Guid.NewGuid, Guid.Empty, Guid.Empty, mCurrentDate, False, Guid.Empty.ToString, LogID.ToString)
			'mInstAssemblyStatus = AssemblyStatus.GetAssemblyStatus(mPeriodListForCompStatus(0).AssemblyStatusID)
			'Session("mInstAssemblyStatus") = mInstAssemblyStatus
			mInstCompStatus.InstallationWONo = mnWO.WONumber


			Dim mtmpCompListOnPartSelection As tmpCompListOnPartSelection = tmpCompListOnPartSelection.GetCompListOnPartSelection(mnWOJobComp.OnPartID.ToString, mnWOJobComp.OnPartNo, mnWOJobComp.OnDescription)
			If mtmpCompListOnPartSelection.Count > 0 Then
				Dim tmpPeriodListForCompStatus As PeriodListForCompStatus = PeriodListForCompStatus.GetPeriodListForCompStatus(mInstCompStatus.AssemblyID, "")
				Dim tmpCompStatus As CompStatus = CompStatus.GetCompStatus(mtmpCompListOnPartSelection(0).ID, tmpPeriodListForCompStatus(0).AssemblyStatusID, mtmpCompListOnPartSelection(0).InstalledOn.ToString)
	...
	Private Sub NewInstRecord(ByVal LogID As Guid, ByVal mCurrentDate As String)
		...
		Else
			...
			Else
				...
				If mRemAssemblyStatus Is Nothing Then
					mInstCompStatus = CompStatus.NewInstallCompStatus(Guid.NewGuid, mAssemblyID, mPeriodListForCompStatus(0).AssemblyStatusID, mCurrentDate, False, Guid.Empty.ToString, LogID.ToString)
				Else
	...
	Private Sub SetRemObject()
		If mRemCompStatus IsNot Nothing Then
	...
	Private Sub SetPage()
		If mRemAssemblyStatus Is Nothing Then
			lblTitle.Text = "Removal / Installation of Component "
		Else
	...
	Public Sub ControlVisibility(ByVal IsForRemoval, ByVal IsForInstall)
		...
		End If
		'End
		If mRemAssemblyStatus IsNot Nothing Then
			'Added by Prashant on 13-Oct-2022 for Fan Blade Distribution  FLYPAL-394
	...
	Private Sub FromLog()
		If CType(Session("FromLog"), Boolean) = True Then
			...
			End If
			If tmpCompStatus IsNot Nothing Then
			...
			If mInstCompStatus.IsValid = True Then
				Try
					...
					If mInstAssemblyStatus.IsSpareAssembly = False Then
						mRegNo = "Reg No. : " & mAssemblyList(New Guid(cmbInstAssemblyList.SelectedValue.ToString)).RegNo  ''& mMachine.RegNo
					End If
					'End
				...
				Catch ex As SqlException
					...
					ElseIf ex.Number = 2627 Then
						...
						Dim mtmpCompStatusList = CompStatusList.GetCompStatusList(CurrentDate:=calInstalledOn.Text, AssemblyID:=Guid.Empty, PartName:=mInstCompStatus.PartName,
						 CompSerialNo:=mInstCompStatus.SerialNo, MachineID:=Guid.Empty.ToString, IsCompRemoved:=True, IsCompPeriodsRequired:=False,
						 ShowForNotInUseAircrafts:=CType(AppSettings("ShowForNotInUseAircrafts"), Boolean)) 'New Added


						If mtmpCompStatusList.count > 0 Then
							phlinks.Visible = True
							MSGBoxCtrl.Show("Installation Save Alert!", "Part No. With Same Serial No. Already Exist in Removed state!!", "Please Install it from Removed Component list", MsgBoxStyle.OkOnly, "Dup")
							Exit Function
						Else
							MSGBoxCtrl.Show("Installation Save Alert!", "Part No. With Same Serial No. Already Installed!!", "", MsgBoxStyle.OkOnly, "")
			...
			Select Case Result1
				...
				Case MsgBoxResult.Ok ''And Session("sender") = ""        'Code Added
					If MSGBoxCtrl.Sender = "ExServiceConfig" Then
						If IsValidInstallation = True Then 'Change
							Session.Remove("IsExpiryServicePresent")

							Dim UpdateOnOffDet As nWOJobCompRemInstList
							UpdateOnOffDet = Session("mnWOJobCompRemInstList")
							UpdateOnOffDet.UpdateCompDet(mnWOJobComp.ID, IsUpdateOn:=True, CompStatusOnID:=mInstCompStatus.ID.ToString)
						...
						End If
					End If
					If MSGBoxCtrl.Sender = "Dup" Then
						phlinks.Visible = True
						lnkInstallSpareComponent.Visible = False
						upnlInstallationDetail.Update()

						Session("InstalledOnMachineID") = mAssemblyList(New Guid(cmbInstAssemblyList.SelectedValue)).MachineID.ToString
						Session("PartNO") = mInstCompStatus.PartName
						Session("SerialNo") = txtInstSerialNo.Text.Trim
						Session("OpenForWOJobCompRemInstNewPage") = "True"
...
#Region " Data Binding "
	Private Sub DataFieldBind()
		...
		If Session("IsFromSpareWO") IsNot Nothing Then
			IsSpareAssembly = IIf(Session("IsFromSpareWO") = "True", True, False)
		End If
		'End

		mAssemblyList = AssemblyList.GetAssemblyListForComboBox(0, , mnWO.WOCloseDateFormatted.ToString, "(SELECT)", IsForSpareAssembly:=IsSpareAssembly)
		...
		If mRemCompStatus IsNot Nothing Then
			If Not mRemCompStatus.AssemblyID.Equals(Guid.Empty) And mnWOJobComp.IsForInstall = True Then
				...
				Session("mRemAssemblyStatus") = mRemAssemblyStatus
				cmbInstAssemblyList.SelectedValue = mRemAssemblyStatus.AssemblyID.ToString

			End If

			txtPartNo.Text = mRemCompStatus.Comp.PartName
			txtSerialNo.Text = mRemCompStatus.Comp.SerialNo
			txtCode.Text = mRemCompStatus.Comp.Code
			txtPosition.Text = mRemCompStatus.Position
			txtWorkOrderNo.Text = mRemCompStatus.RemovalWONo
			txtNote.Text = mRemCompStatus.RemovalRemark
			chkExpired.Checked = mRemCompStatus.IsExpired
			txtRemDoneBy.Text = mRemCompStatus.RemDoneBy
			chkRemCompStatusFanBladeMonitoring.Checked = mRemCompStatus.IsFanBladeDistribution
			txtRemCompStatusFanBladePosition.Text = mRemCompStatus.FanBladePosition
			txtRemCompStatusMomentWeight.Text = mRemCompStatus.MomentWeight
			txtRemCompStatusBalanceScrew.Text = mRemCompStatus.BalanceScrew
		End If


		If mInstCompStatus IsNot Nothing And mnWOJobComp.IsForInstall = True Then
			dgInstallationValue.DataSource = mInstCompStatus.CompStatusPeriods
		...
		If cmbReason.Items.Contains(New System.Web.UI.WebControls.ListItem(mnWOJobComp.RemovalReasonName, mnWOJobComp.RemovalReasonID.ToString)) Then
		...
		If cmbInstPartNo.Items.Contains(New System.Web.UI.WebControls.ListItem(mnWOJobComp.OnPartNo, mnWOJobComp.OnPartID.ToString)) Then
			cmbInstPartNo.SelectedValue = mnWOJobComp.OnPartID.ToString
		Else
			cmbInstPartNo.SelectedValue = Guid.Empty.ToString
		End If
		txtInstSerialNo.Text = mnWOJobComp.OnSerialNo
		txtInstDescription.Text = mnWOJobComp.OnDescription
		txtInstPartNo.Text = mnWOJobComp.OnPartNo
		txtInstPosition.Text = mnWOJobComp.OnPosition

		If mRemCompStatus IsNot Nothing Then
			If cmbInstATAChapter.Items.Contains(New System.Web.UI.WebControls.ListItem(mRemCompStatus.ATAChapter, mRemCompStatus.ATAID.ToString)) And mnWOJobComp.IsForInstall = True Then
...
#Region " Events "
	Private Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
		...
		If Not IsPostBack Then
			...
			chkRemoval.Checked = mnWOJobComp.IsForRemoval
			mIsRemoval = chkRemoval.Checked

			chkInstallation.Checked = mnWOJobComp.IsForInstall
			mIsInstall = chkInstallation.Checked


			Session("IsRemoval") = mIsRemoval
			Session("mIsInstall") = mIsInstall


			' calInstalledOn.Text = mRemCompStatus.RemovedOnFormatted.ToString
			If mnWOJobComp.IsForInstall Then calInstalledOn.Text = mnWO.WOCloseDateFormatted.ToString()


			If Session("IsFromAddPeriod") = False And CType(Session("FromLog"), Boolean) = False Then
				' NewRecord(Guid.Empty, Today.Date.ToString)

				NewRecord(Guid.Empty, mnWO.WOCloseDateFormatted.ToString)
			Else
		...
		If Page.IsValid Then
			Try
				If chkRemoval.Checked And SaveRemoval() = True Then
					...
					Dim UpdateOnOffDet As nWOJobCompRemInstList
					UpdateOnOffDet = Session("mnWOJobCompRemInstList")
					UpdateOnOffDet.UpdateCompDet(mnWOJobComp.ID, IsUpdateOff:=True)

					IsValidRemoval = True
					Dim mRegNo As String = ""
					If mInstAssemblyStatus.IsSpareAssembly = False Then
						''mRegNo = "Reg No. : " & mMachine.RegNo
						mRegNo = "Reg No. : " & mAssemblyList(New Guid(cmbInstAssemblyList.SelectedValue.ToString)).RegNo
					End If
					'End
					...
					If chkInstallation.Checked = True And SaveInstall() = True Then
						Try
							...
							If IsExpiryServicePresent Then
								MSGBoxCtrl.Show("Alert!", "Expiry Service are not configured on newly installed component.", "Please configure Expiry service manually", MsgBoxStyle.OkOnly, "ExServiceConfig")
								Exit Sub
							End If
						...
						Catch ex As Exception
							'Throw ex
						End Try
					ElseIf chkInstallation.Checked = True And SaveInstall() = False Then 'Change

						If phlinks.Visible = False Then
			...
			Finally
				...
				'Response.Redirect(Request.QueryString("BackPage"))
			End Try
			If IsValidRemoval = True And IsValidInstallation = True Then 'Change
				Session.Remove("IsExpiryServicePresent")




				'Added by Saylee on 26-Apr-2024
				...
				'End If
				'End
				Session.Remove("mRemCompStatus")
				Session.Remove("mInstCompStatus")
				Response.Redirect(Request.QueryString("BackPage"))
			Else

				If IsValidInstallation = False Then
					MSGBoxCtrl.Show("Alert..!!", "There was an issue in Installation.", "Please try again later.", MsgBoxStyle.OkOnly, "")
					Exit Sub
				ElseIf IsValidRemoval = False Then
					MSGBoxCtrl.Show("Alert..!!", "There was an issue in Removal.", "Please try again later.", MsgBoxStyle.OkOnly, "")
					Exit Sub
				End If
			End If
	...
	Private Sub btnBack_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBack.Click
		...
		Session.Remove("mRemCompStatus")
		Session.Remove("mInstCompStatus")
		Session.Remove("IsFromAddPeriod")
	...
	Private Sub chkRemoval_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
		...
		ControlVisibility(mIsRemoval, mIsInstall)
	End Sub
	Private Sub cmbInstAssemblyList_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbInstAssemblyList.SelectedIndexChanged
		SetAssemblyPeriod()
	End Sub
			...
			If chkInstallation.Checked = True And SaveInstall() = True Then
				Try
					...
					If (Not (mRemCompStatus Is Nothing)) Then
						...
						End If
					End If
					Session.Remove("OpenForWOJobCompRemInstNewPage")
					If IsExpiryServicePresent Then
						MSGBoxCtrl.Show("Alert!", "Expiry Service are not configured on newly installed component.", "Please configure Expiry service manually", MsgBoxStyle.OkOnly, "ExServiceConfig")
						Exit Sub
					End If
					...
					btnSave.ToolTip = "Installation finished successfully"
					upnlButtons.Update()
...
#Region " Event "
	Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
		...
		For I = 0 To LHCount - 1
			If I = 0 Then
				ReportDetails.Add(New rptStatus(, 1, , lblPartNo.Text,
	txtPartNo.Text, , , , , , , , , , , , , , , , , "",
	"", "", , ""))
			ElseIf I = 1 Then
   ...
   "", "", , ""))
		...
		If temp1 < RHCount1 Then
			...
			dgRemovalValue.Columns.Item(0).HeaderText, dgRemovalValue.Columns.Item(1).HeaderText,
		...
		For m = 0 To TotalCount1 - 1
			If m = 0 Then
				If m < RHCount1 Then
					ReportDetails.Add(New rptStatus(, 4, , , , lblWorkOrderNo.Text,
					 txtWorkOrderNo.Text, , , , , , , , , , , , , , , ,
				CType(Me.mRemCompStatus.CompStatusPeriods(m).PeriodName, String),
				CType(Me.mRemCompStatus.CompStatusPeriods(m).CompRemovalValueFormatted, String), ,
			...
			ElseIf m = 1 Then
				If m < RHCount1 Then
					ReportDetails.Add(New rptStatus(, 4, , , , lblReason.Text,
