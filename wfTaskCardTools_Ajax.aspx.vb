...
Public Class wfTaskCardTools_Ajax
...
#Region " Business Methods "
    ...
    End Sub
    Private Sub SetPage()
        If Session("ToolEdit") Then
            lblTitle.Text = "Tool [" & mTaskCard.TaskCardTools.CurrentItem.PartNo & " ]"
        Else
    ...
    Private Function setObject() As Boolean
        'mTaskCard.TaskCardTools.CurrentItem.ItemID = New Guid(PartID) 'New Guid(cmbItemList.SelectedValue.ToString)
        ItemID = IIf(txtItemList.Text = "", Guid.Empty, mItemListForCombo(PartNo).ID)
        mTaskCard.TaskCardTools.CurrentItem.ItemID = ItemID
        mTaskCard.TaskCardTools.CurrentItem.PartNo = Trim(txtPartNo.Text)
        mTaskCard.TaskCardTools.CurrentItem.Description = Trim(txtDescription.Text)
        mTaskCard.TaskCardTools.CurrentItem.RequiredQty = Val(txtReqQty.Text)
        mTaskCard.TaskCardTools.CurrentItem.Remark = Trim(txtRemark.Text)
        Return True
    End Function
    ...
    Private Sub SaveTaskCard()
        Try
            mTaskCard.Save()
            MarkLog(Util.Action.Save, "TaskCard", "Task Card Tool : " + Chr(13) + "Part No. : " + txtPartNo.Text + "," + "Description : " + txtDescription.Text + "," + "Qty. : " + txtReqQty.Text, Util.ErrorType.NoError, mTaskCard.ID, EventLogID)
...
#Region " Data Binding "
    Private Sub DataFieldBind()
        ...
        DataBind()
        txtItemList.Text = IIf(Not mTaskCard.TaskCardTools.CurrentItem.ItemID.Equals(Guid.Empty), mTaskCard.TaskCardTools.CurrentItem.PartNo + " [" + mTaskCard.TaskCardTools.CurrentItem.Description + "]", "")
        hdnpartId.Value = IIf(Not mTaskCard.TaskCardTools.CurrentItem.ItemID.Equals(Guid.Empty), mTaskCard.TaskCardTools.CurrentItem.ItemID.ToString, "{00000000-0000-0000-0000-000000000000}")
    End Sub
...
#Region " Events "
    ...
    Private Sub btnOK_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnOK.Click
        ...
        SetPartID()
        If Session("ToolEdit") = False Then
            If Trim(txtPartNo.Text) <> "" And Not mTaskCard.TaskCardTools.Contains(mTaskCard.TaskCardTools.CurrentItem.ID, mTaskCard.ID, Trim(txtPartNo.Text)) Then
                setObject()
            ...
            ElseIf Trim(txtPartNo.Text) <> "" And mTaskCard.TaskCardTools.Contains(mTaskCard.TaskCardTools.CurrentItem.ID, mTaskCard.ID, Trim(txtPartNo.Text)) Then
                MSGBoxCtrl.show(MSGBox.Message_title.Duplicate, MSGBox.Message_text.Duplicate, "", MsgBoxStyle.OkOnly, "")
                mTaskCard.TaskCardTools.CurrentItem.Description = ""
                Exit Sub
            Else
                setObject()
                If Not CustomValidate1() Then
        ...
        Else
            ...
            If mTaskCard.TaskCardTools.CurrentItem.IsDirty Then
                ...
                If EitherPartNoOrItemID = True Then
                    ...
                    ElseIf Not mTaskCard.TaskCardTools.Contains(ItemID) Then
                        If Not CustomValidate1() Then
                            mTaskCard = clnTaskCard
                            Session("mTaskCard") = mTaskCard
                            Session("wfTaskCard.TaskCard") = clnTaskCard
    ...
    Private Sub btnClose_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnClose.Click
        If mTaskCard.TaskCardTools.CurrentItem.IsNew And Not Session("ToolEdit") = True Then mTaskCard.TaskCardTools.Remove(mTaskCard.TaskCardTools.CurrentItem)
        Session("ToolEdit") = False
        upnlValidationsummary.Update()
        Session.Remove("PartID")
        Session.Remove("mItemListForCombo")
