...
Public Class wfCompMonitorModStatusListNew_Ajax
...
#Region " Business Methods "
    ...
    Private Sub NewRecord()
        ...
        mCompMonitorModStatus = CompMonitorModStatus.NewCompMonitorModStatus(Guid.NewGuid, mCompStatus.CompID, mAssemblyStatus.ID, mCompStatus.AsOnDate, mCompStatus.Comp.PartID, mAssemblyStatus.Assembly.ModelID, mCompStatus.ID, mMachine.HourType)
        ...
        If (Not User.IsInRole("MachineNew") And mMachine.IsNew) Or (Not User.IsInRole("MachineEdit") And Not mMachine.IsNew) Then
            MarkLog(Util.Action.[New], "Assembly Modification Status", User.Identity.Name & " is not Authorized User to add new", Util.ErrorType.HandledError, Guid.Empty, EventLogID)
            MSGBoxCtrl.show(MSGBox.Message_title.Authorization, MSGBox.Message_text.Authorization, "", MsgBoxStyle.OkOnly, "")
            Exit Sub
        End If
        MarkLog(Util.Action.[New], "Assembly Modification Status", "Assembly Info : " & mAssemblyStatus.ModelName & " " & mAssemblyStatus.Assembly.SerialNo & " Part Info : " & mCompStatus.Comp.PartName & "  " & mMachine.AssemblyStatus.Assembly.SerialNo, Util.ErrorType.NoError, mCompMonitorModStatus.ID, EventLogID)
        'Response.Redirect("wfPartMonitorModList.aspx?BackPage=wfCompMonitorModStatusListNew.aspx")
        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "OpenScript", "openledgersame('wfPartMonitorModList_Ajax.aspx?BackPage=Index.aspx');", True)
    End Sub
    ...
    Private Sub SetPage()
        If Not mMachine Is Nothing Then
            lblTitle.Text = "Add New Modifications for Aircraft " + mMachine.RegNo
        End If
    End Sub
#End Region
