Public Class APPNotificationOrderDet
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If Not IsPostBack Then
                ...
                OrderID = New Guid(Request.QueryString("ModuleID"))

                Dim mAPP_UserNotification As APP_UserNotification = APP_UserNotification.GetAPP_UserNotification(NotificationID)

                Dim mApp_Order As Order = Order.GetOrder(OrderID)

                Session("mApp_Order") = mApp_Order

                Session("APPNotificationFlightDet.NotificationID") = NotificationID

                With mApp_Order
                    spnOrderNo.InnerText = .OrderNo
                    spnDate.InnerText = .OrderDateFormatted
                    spnVendor.InnerText = .VendorName
                    spnAuthorizedBy.InnerText = .AuthorizedBy
                    spnRemark.InnerText = .Remark
                End With
                DataBind()


                'Dim mApp_OrderList As OrderList
                'mApp_OrderList = OrderList.GetOrderList("", Text:=mApp_Order.Text, No:=mApp_Order.No, Amend:=mApp_Order.Amend, _
                '                                        FromDate:="1/1/1900", ToDate:="1/1/4400")
                'grdOrderList.DataSource = mApp_OrderList
