Public Class wfServicedInspectedDueReport_Ajax
...
#Region " Helper Methods "
    ...
    Private Sub SetReport(ByVal IsExcel As Boolean)
        ...
        mCompanyDetail.WebSite, "Equipment Maintenance Due Report", Search1, Search2, Search3, Search4, Search5, AppSettings("Product Version"), _
        AppSettings("SINote"), SearchStr6:=StrCategory, SearchStr10:=AppSettings("Logo"), SearchStr11:=cmbListOfItemServiceInspections.SelectedItem.Text)

        Obj = ConditionCheckDueReport.GetConditionCheckDueReport(, ToDate, PartNo, Description, "", mStoreID, RangeDate, mCategoryID.ToString, _
            IsConditionCheckServicedInspected:=2, ServiceInspectionsID:=cmbListOfItemServiceInspections.SelectedValue.ToString)
        ...
        ElseIf IsExcel = True Then  'Excel format
            ...
            End If
            If ds.Tables("ConditionCheckDueReport").Columns.Contains("CondCheckIntervalIn") Then
                ds.Tables("ConditionCheckDueReport").Columns("CondCheckIntervalIn").ColumnName = "Serviced Inspected Interval In"
            End If
            ...
            If ds.Tables("ReportData").Columns.Contains("SearchStr11") Then
                ds.Tables("ReportData").Columns("SearchStr11").ColumnName = "Service/Inspection"
            End If

            Dim dsNew As New DataSet
            dsNew.Clear()
            ds.Tables("ReportData").TableName = "Searching Criteria"
			ds.Tables("ConditionCheckDueReport").TableName = "Equipment Maintenance Due Report"
			Session("ExcelFileName") = "Equipment Maintenance Due Report"
			dsNew = ds
			Session("dsNew") = dsNew
            ScriptManager.RegisterStartupScript(Me, Me.GetType(), "openFile", "openFile();", True)
            'Added by Prashant on 19-Jan-2021
            MarkLog(Util.Action.Print, "ConditionCheckDueReport", "Export To Excel " + mSearchingCriteria, Util.ErrorType.NoError, Guid.Empty, EventLogID)
        End If
    End Sub
#End Region
