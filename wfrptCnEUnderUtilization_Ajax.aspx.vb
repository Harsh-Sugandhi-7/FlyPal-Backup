...
Public Class wfrptCnEUnderUtilization_Ajax
...
#Region " Helper Methods "
    ...
    Public Sub SetValues()
        ...
        lblFrom.Text = "As On Date : " & txtFromDate.Text
    ...
    Private Sub SetReport(Optional ByVal IsExcel As Boolean = False)
        ...
        Dim rpt As ConsumableAndExpendableUnderUtilizationReport
        Dim mCompanyDetail As New CompanyDetail
        SetValues()

        myReport = New crptConsumableAndExpendablesUnderUtilization


        rpt = ConsumableAndExpendableUnderUtilizationReport.GetList(AsOnDate:=txtFromDate.Text, StartingDate:=AppSettings("StartingDateForCnEConsideration").ToString(), MachineID:=cmbAircraftList.SelectedValue.ToString _
                                          , ReqID:=cmbRequisitionText.SelectedValue.ToString, ItemName:=PartNo, ItemDesc:=Description, NoOfDaysToConsider:=CInt(AppSettings("NoOfDaysForCnEConsideration")))
        If rpt.Count <= 0 Then
            MSGBoxCtrl.show(MSGBox.Message_title.NoRecordFound, MSGBox.Message_text.NoRecordFound, "There is no record for this search criteria", MsgBoxStyle.OkOnly, "")
            Exit Sub
        Else
            RecentMenuEvent.RecentMenuItemEvent(User.Identity.Name, 1391)
        End If

        rpt.Sort("MismatchQty", ComponentModel.ListSortDirection.Descending)

        Dim Report As New ReportData(mCompanyDetail.CompanyName, mCompanyDetail.Address, _
               mCompanyDetail.Tel1, mCompanyDetail.Tel2, mCompanyDetail.Fax, mCompanyDetail.Email, _
               mCompanyDetail.WebSite, "Consumables & Expendables (C&E) Under Utilization Report", txtFromDate.Text, "", IIf(cmbAircraftList.SelectedIndex > 0, cmbAircraftList.SelectedItem.Text, ""), IIf(cmbRequisitionText.SelectedIndex > 0, cmbRequisitionText.SelectedItem.Text, ""), _
        ...
        MarkLog(Util.Action.Print, "ConsumablesAndExpendablesUnderUtilization", "", Util.ErrorType.NoError, Guid.Empty, EventLogID)
    End Sub
