'Added By Vikrant On 01-Sept-2015

Public Class wfFindingRectificationList_Ajax
...
#Region " Variable Declaration "
    ...
    Dim ShowOpenClosed As Boolean  'Added by Ajay 18-Nov-2022  
#End Region
...
#Region " Helper Methods "
    Private Sub GetSession()
        ...
        ShowOpenClosed = Session("ShowOpenClosed") 'Added by Ajay 18-Nov-2022
    End Sub
    Private Sub RemoveSession()
        ...
        Session.Remove("ShowOpenClosed") 'Added by Ajay 18-Nov-2022
    End Sub
    Private Sub ClearAll()
        OpenFrom = Session("OpenFrom")
        If InStr(Session("MiddleFrame"), "wfFindingRectificationList_Ajax.aspx?OpenFrom=" & OpenFrom) <= 0 Then
            RemoveSession()
        End If
    End Sub
    ...
    Private Sub DeleteRecord(ByVal mID As Guid)
        ...
        Session("mAuditExecution") = mAuditExecution
    End Sub
    Private Sub FindNow(Optional ByVal FromDate As String = "", Optional ByVal ToDate As String = "", Optional ByVal SearchTxt As String = "", Optional ByVal ShowOpenClosed As Boolean = False) 'Ajay Added by ShowOpenClosed 18-Nov-2022
        ...
        SetGrid()
        lblResult.Text = "List of Audit Closing as per criteria :" & mAuditExecutionList.Count & " Record(s) found."
    End Sub
                    ...
                    If MSGBoxCtrl.Sender = "Delete" Then
                        Try
                            ...
                            If Not mFileAttach Is Nothing Then
                                ...
                                End If
                            End If
                            MarkLog(Util.Action.Delete, "Audit Execution", mExecutionDetail, Util.ErrorType.NoError, mAuditExecution.ID, EventLogID) 'Changed by Vikrant on 22-July-2011
                            SetControl()
                            setPeriod(SearchIdx)
                            SetGrid()
                            upnlGrid.Update()
                        ...
                        Catch ex As SqlException
                            ...
                            ElseIf ex.Number = 547 Then
                                ...
                                MarkLog(Util.Action.Delete, "Audit Execution", "Can't delete :" & mExecutionDetail & " is Currently in use", Util.ErrorType.NoError, mAuditExecution.ID, EventLogID)
                            End If
    ...
    Private Sub SetGrid()
        ...
        '    B = CType(Me.dgAuditExecution.Rows.Item(j).Cells(12).Text, Boolean)
        '    If B = False Then
        '        dgAuditExecution.Rows.Item(j).Cells(11).Enabled = False
        '    End If
        'Next
    ...
    Private Sub SetTitle()
        If OpenFrom = 1 Then 'Finding Rectification Form
            ...
            btnCloseTop.ToolTip = "Click to close Finding Rectification List screen" 'Ajay 10-11-2023
            lblResult.Text = "List of Finding Rectification as per criteria :" & mAuditExecutionList.Count & " Record(s) found."

        ElseIf OpenFrom = 2 Then 'Audit Closing Form
            lblTitle.Text = "Audit Closing"
            btnCloseTop.ToolTip = "Click to close Audit Closing List screen" 'Ajay 10-11-2023
            lblResult.Text = "List of Audit Closing as per criteria :" & mAuditExecutionList.Count & " Record(s) found."
        End If
    End Sub
#End Region
...
#Region " Events "
    Private Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not IsPostBack Then
            ...
            Session("MiddleFrame") = "wfFindingRectificationList_Ajax.aspx?OpenFrom=" & OpenFrom
            setPeriod(0)
            DataFieldBind()
            SetControl()
    ...
    Private Sub dgAuditExecution_RowCommand(ByVal source As System.Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles dgAuditExecution.RowCommand
        Select Case e.CommandName
            Case "EditRec"
                If (Not User.IsInRole("FindingRectificationView") And Not User.IsInRole("FindingRectificationEdit")) Then
                ...
                'Added by Saylee on 19-Sep-2022, to restrict user if has no deparment rights
                Dim mUser As User = SI.UTILITY.User.GetUser(User.Identity.Name)
                If mUser.UserEmployeeDepartments.Contains(mAuditExecution.AuditDepartmentID, "") Or mUser.UserStores.Contains(mAuditExecution.AuditStoreID, "") Or mUser.UserMachines.Contains(mAuditExecution.AuditMachineID, "") Then
                    Dim errMsg As String

                    If mUser.UserEmployeeDepartments.Contains(mAuditExecution.AuditDepartmentID, "") Then
                        errMsg = "User is not authorised person to rectify Findings of " + mAuditExecution.AuditOnNameDetail + " department."
                    ElseIf mUser.UserStores.Contains(mAuditExecution.AuditStoreID, "") Then
                        errMsg = "User is not authorised person to rectify Findings of " + mAuditExecution.AuditOnNameDetail + " Store."
                    ElseIf mUser.UserMachines.Contains(mAuditExecution.AuditMachineID, "") Then
                        errMsg = "User is not authorised person to rectify Findings of " + mAuditExecution.AuditOnNameDetail + " aircraft."
                    End If

                    'MSGBoxCtrl.show(MSGBox.Message_title.Authorization, MSGBox.Message_text.Authorization, errMsg, MsgBoxStyle.OkOnly, "")
                    MSGBoxCtrl.show("Not Authorized!", errMsg, "", MsgBoxStyle.OkOnly, "")
                    Exit Sub
                End If
                    '*******************************************************

                    mExecutionDetail = "Audit No : " + mAuditExecution.AuditNo + " Start Date : " + mAuditExecution.StartDateFormatted + " Lead Auditor : " + mAuditExecution.AuditorName
                    MarkLog(Util.Action.Edit, "Audit Execution", mExecutionDetail, Util.ErrorType.NoError, mAuditExecution.ID, EventLogID)
                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "OpenScript", "openledgersame('wfFindingRectification_Ajax.aspx?BackPage=index.aspx');", True)
            Case "View"
    ...
    Protected Sub txtSearchText_TextChanged(sender As Object, e As EventArgs) Handles txtSearchText.TextChanged, txtFromDate.TextChanged, txtToDate.TextChanged, ChkOpenColsed.CheckedChanged
        ...
        FindNow(FromDate, ToDate, SearchTxt, ShowOpenClosed) 'Ajay 18-Nov-2022 ShowOpenClosed
        upnlGrid.Update()
    End Sub
