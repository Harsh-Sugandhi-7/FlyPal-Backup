Public Class wfExportInvoiceTerm_Ajax
...
#Region " Business Properties "
    ...
    Private Sub setTerms()
        ...
        While i < mTerms.Count
            If mExportInvoice.ExportinvoiceTerms.Contains(mTerms.Item(i).ID) = True Then
    ...
    Private Sub DataFieldBind()
        mTerms = Terms.GetTerms(mExportInvoice.ID, 10)
        setTerms()
        ...
        While i < mTerms.Count
            If mTerms.Item(i).IsDirty = True Then
                If mTerms.Item(i).IsSelected = True Then
                    If mExportInvoice.ExportinvoiceTerms.Contains(mTerms.Item(i).ID) = False Then
                        mExportInvoice.ExportinvoiceTerms.Add(mTerms.Item(i).ID)
                        mExportInvoice.ExportinvoiceTerms.CurrentItem.Terms = mTerms.Item(i).Terms
                        mExportInvoice.ExportinvoiceTerms.CurrentItem.TermID = mTerms.Item(i).ID
                    End If
                Else
                    mExportInvoice.ExportinvoiceTerms.Remove(mTerms.Item(i).ID, "")
                End If
            End If
            i = i + 1
        End While
