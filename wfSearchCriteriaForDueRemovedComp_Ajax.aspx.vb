...
Public Class wfSearchCriteriaForDueRemovedComp_Ajax
...
#Region " Variable Declaration "
    ...
    Dim CompName, PartName As String
    Dim Comp1, Part1 As String
    ...
    Public mRemovedCompListForDue As RemovedCompListForCombo
    ...
    Dim IsSpareComp As Boolean = False
...
#Region " Helper Methods "
    ...
    Private Sub GetSession()
        ...
        mRemovedCompListForDue = Session("mRemovedCompListForDue")
    ...
    Private Sub SetSession()
        ...
        Session("mRemovedCompListForDue") = mRemovedCompListForDue
    ...
    Private Sub ClearAll()
        ...
        If Session("MiddleFrame") <> "wfSearchCriteriaForDueRemovedComp_Ajax.aspx?DueType=" & DueType Then
            ...
            'Session.Remove("mModelListAutoComplete")
            'Session.Remove("mAssemblyTypeList")
            Session.Remove("mRemovedCompListForDue")
        End If
        mIsExcel = False
    End Sub
    Public Sub CustomValidate(ByVal s As Object, ByVal e As ServerValidateEventArgs)
        ...
        mCompID = IIf(CompID.Value.Length > 0, CompID.Value, Guid.Empty.ToString)

        If custValidator.ControlToValidate = "cmbComponent" Then
            If mCompID = Guid.Empty.ToString Then
    ...
    Private Sub SetValues()
        ...
        If mCompID = Guid.Empty.ToString Then
            '  ModelName = "{00000000-0000-0000-0000-000000000000}"
            Comp1 = "(ALL)"
            CompName = "{00000000-0000-0000-0000-000000000000}"
            lblComp1.Text = ""
            'lblModelName1.Text = ""
            IsSpareComp = False  'Added by Shital on 19-oct-2020
        Else

            CompName = mCompID.ToString
            Comp1 = txtSerialNo.Text
            lblComp1.Text = "Comp Name : " & Comp1  'Added Code
            mRemovedCompListForDue = RemovedCompListForCombo.GetCompList(txtFromDate.Text.ToString, txtComp.Text, txtSerialNo.Text)
            IsSpareComp = mRemovedCompListForDue(New Guid(mCompID.ToString), "").IsSpareComp
        End If

        If mPartID = Guid.Empty.ToString Then
            '  ModelName = "{00000000-0000-0000-0000-000000000000}"
            Part1 = "(ALL)"
            PartName = "{00000000-0000-0000-0000-000000000000}"
            lblPart1.Text = ""
            'lblModelName1.Text = ""

        Else

            PartName = mPartID.ToString
            Part1 = txtComp.Text
            lblPart1.Text = "Part Name : " & Part1  'Added Code
        ...
        mEventLogDetails = lblDateRangeFrom.Text + ", " + lblPart1.Text + ", " + lblComp1.Text + ", " + DueLimits
    End Sub
    ...
    Public Function ReportDetail(IsExcel As Boolean) As ReportMaintenanceDetailList
        Try
            ...
            ' mMachineList = MachineList.GetMachineListDueMonitoringStatusForRemoveComp(AsonDate, mDueLimits, , , , , mPerDayLimits, , False, False, False, , , , IsSerSelect, IsInsSelect, IsModSelect, Val(txtForecastingLimit.Text), True, SkipIsForInventoryAircarft:=True, CompID:=CompName, PartName:=txtComp.Text.Split("(")(0).Trim, IsSpareComp:=IsSpareComp)
            Dim IsSpareOrRemovedComponent As Integer = 0
            If IsSpareComp = True Then
                IsSpareOrRemovedComponent = 1
            Else
                If CompName = "" Then
                    IsSpareOrRemovedComponent = 0
                End If
                IsSpareOrRemovedComponent = 2
            End If

            Dim mCompStatusList As CompStatusList = CompStatusList.GetCompStatusList(Guid.Empty, CurrentDate:=AsonDate, DueLimits:=mDueLimits, IsCompRemoved:=Not IsSpareComp, InspStatusRequired:=IsInsSelect, ServiceStatusRequired:=IsSerSelect, ModStatusRequired:=IsModSelect, CompID:=CompName, PartName:=txtComp.Text.Split("(")(0).Trim, IsForSpareComponent:=IsSpareComp, IsSpareOrRemovedComponent:=IsSpareOrRemovedComponent, IsSpareComponentAlsoRequired:=True, CompSerialNo:=txtSerialNo.Text.ToString)
            ...
            If IsSerSelect = True Then
                ...
                For Each ObjCompStatus In mCompStatusList 'For Each ObjCompStatus In ObjAssemblyStatus.CompStatusList
                        ...
                        If ServiceTypeID.Contains(ObjCompMonitorServiceStatus.PartMonitorServiceTypeID) Then
                            If ObjCompMonitorServiceStatus.CompMonitorServiceStatusPeriodList.Count > 0 Then
                                If (ObjCompMonitorServiceStatus.IsApplicable = True) And (Not (ObjCompMonitorServiceStatus.MonitorTypeID = 1 And ObjCompMonitorServiceStatus.IsCompleted = True)) Then
                                    ...
                                    AssemblyModel = ObjCompStatus.Model
                                    AssemblySerialNo = ObjCompStatus.AssemblySerialNo & IIf(IsExcel, Chr(10), vbCrLf) & ObjCompStatus.AssemblyPosition
                                    MinimumRemainingValue = ObjCompMonitorServiceStatus.MinimumRemainingValue
                                    AssemblyTypeID = ObjCompStatus.AssemblyTypeID
                                    ...
                                    For Each ObjCompMonitorServiceStatusPeriod In ObjCompMonitorServiceStatus.CompMonitorServiceStatusPeriodList
										...
										End If
                                    Next
                                    AssemblyID = ObjCompStatus.AssemblyID
                                    AssemblyType = ObjCompStatus.AssemblyType
                                    RegNo = ObjCompStatus.RegNo  'ObjMachine.RegNo
                                    'Rajnish 08-08-2008
    ...
    Private Sub SetExcel(ReportMaintenanceDetails As ReportMaintenanceDetailList, SearchingCriteria As ReportData, ReportName As String)
        ...
        For i As Integer = 0 To ds.Tables("ExcelReport").Columns.Count - 1
            ...
            If ds.Tables("ExcelReport").Columns(i).ColumnName = "SearchStr3" Then
                ds.Tables("ExcelReport").Columns(i).ColumnName = "Part No."
            End If
            If ds.Tables("ExcelReport").Columns(i).ColumnName = "SearchStr5" Then
                ds.Tables("ExcelReport").Columns(i).ColumnName = "Serial No."
            End If
        Next
        ...
        MarkLog(Util.Action.Print, "Due-RemovalComp", "Export To Excel " + mEventLogDetails, Util.ErrorType.NoError, Guid.Empty, EventLogID)
    End Sub
    Private Sub SetReport(Optional ByVal ByMail As Boolean = False, Optional ByVal ByExcel As Boolean = False)
        Try
            ...
            If DueType = 1 Then
                ...
                If ((AppSettings("ClientCode") = "Heligo")) Then
                    ReportName = "Maintenance Forecast for Removed Components"
                Else
                    ReportName = "Maintenance Status Report for Removed Components"
                End If
       ...
       mCompanyDetail.WebSite, ReportName, searchstr, "Component", Part1, AppSettings("ClientCode"), Comp1, AppSettings("Product Version"), AppSettings("SINote"), searchstr6, OperatorName, searchstr8, "", AppSettings("Logo"), AppSettings("FormNo"), mModuleList.Item("Due-RemovalComp").FormRevisionNo)
            'Replace AppSettings("RevisionNo") with mModuleList.Item("Due-RemovalComp").FormRevisionNo in Report Data  by Shital
            ...
            If (ByMail = True And ReportMaintenanceDetails.Count <= 0) Then
                SendMailFile.SendMailFile(, Thread.CurrentPrincipal.Identity.Name, ReportName, ReportName, "There is no record for this search criteria.", "", _
                    ...
                    SmtpHost:=mModuleList.Item("Due-RemovalComp").SmtpHost, SmtpPort:=mModuleList.Item("Due-RemovalComp").SmtpPort, _
                    SmtpUser:=mModuleList.Item("Due-RemovalComp").SmtpUser, SmtpPassword:=mModuleList.Item("Due-RemovalComp").SmtpPassword)
                Exit Sub
            End If
            '11-Sep-2008-------------------------------
            If Not mIsPreview Then
                ...
                If ByMail Then
                    SendMailFile.SendMailFile(Session("CrystalReport"), Thread.CurrentPrincipal.Identity.Name, ReportName, ReportName, _
                                              " For " + lblDateRangeFrom.Text + ", " + lblPart1.Text, "", Session("ToSendMailIDs"), _
                ...
                Else
                    ...
                    MarkLog(Util.Action.Print, "Due-RemovalComp", mEventLogDetails, Util.ErrorType.NoError, Guid.Empty, EventLogID)
                End If
...
#Region " Data Binding "
    Public Sub DataFieldBind()
        ...
        'mModelList = ModelList.GetModelList(mAssemblyTypeList(0).ID, "", Guid.Empty.ToString, Guid.Empty.ToString, "(SELECT)")
        'cmbModel.DataSource = mModelList
        'Session("mModelList") = mModelList

        'mRemovedCompListForDue = RemovedCompListForCombo.GetCompList(txtFromDate.Text, "(ALL)")
        'cmbComp.DataSource = mRemovedCompListForDue

        'Session("mRemovedCompListForDue") = mRemovedCompListForDue
        DataBind()

        'If mModelList.Count > 1 Then cmbModel.Enabled = True Else cmbModel.Enabled = False
        'If mRemovedCompListForDue.Count > 1 Then cmbComp.Enabled = True Else cmbComp.Enabled = False
    End Sub
...
#Region "Events"
    ...
    '    mRemovedCompListForDue = RemovedCompListForCombo.GetCompList(New Guid(cmbModel.SelectedValue.ToString), txtFromDate.Text, "(ALL)")
    '    cmbComp.DataSource = mRemovedCompListForDue
    '    cmbComp.DataBind()
    '    If mRemovedCompListForDue.Count > 1 Then cmbComp.Enabled = True Else cmbComp.Enabled = False
    '    Session("mRemovedCompListForDue") = mRemovedCompListForDue
    ...
    '    mModelList = ModelList.GetModelList(CType(cmbAssemblyType.SelectedValue.ToString, Integer), "", Guid.Empty.ToString, Guid.Empty.ToString, "(SELECT)")
    '    cmbModel.DataSource = mModelList
    '    Session("mModelList") = mModelList
    '    cmbModel.DataBind()
    '    If mModelList.Count > 1 Then cmbModel.Enabled = True Else cmbModel.Enabled = False
    ...
    Protected Sub btnByMail_Click(sender As Object, e As EventArgs) Handles btnByMail.Click
        ...
        Session("UserEmailID") = mModuleList.Item("Due-RemovalComp").SendToMailID
        Session("UserCcEmailID") = mModuleList.Item("Due-RemovalComp").SendCCMailID
        '--------------------------
        Dim Str As String
        Str = "OpenByMaiWindow();"
...
#Region "Service Methods"
    ...
    Public Shared Function GetPartNoList(ByVal prefixText As String, ByVal count As Integer, ByVal contextKey As String) As String()
        Dim AsOnDate As String = contextKey

        Dim mRemovedCompListForDue As RemovedCompListForCombo
        mRemovedCompListForDue = RemovedCompListForCombo.GetPartList(AsOnDate.ToString, prefixText)

        If count = 0 Then
            Return (From c As RemovedCompListForCombo.RemovedCompListForComboInfo In mRemovedCompListForDue
              Select AjaxControlToolkit.AutoCompleteExtender.CreateAutoCompleteItem(c.PartDesc, c.PartID.ToString())).ToArray
        Else
            Return (From c As RemovedCompListForCombo.RemovedCompListForComboInfo In mRemovedCompListForDue
                   Select AjaxControlToolkit.AutoCompleteExtender.CreateAutoCompleteItem(c.PartDesc, c.PartID.ToString())).Take(count).ToArray
        End If
    End Function
    <System.Web.Services.WebMethodAttribute(), System.Web.Script.Services.ScriptMethodAttribute()>
    Public Shared Function GetSerialNoList(ByVal prefixText As String, ByVal count As Integer, ByVal contextKey As String) As String()
        Dim AsOnDate As String = contextKey.Split("=")(0)
        Dim PartNameDesc As String = contextKey.Split("=")(1)
        Dim PartName As String = PartNameDesc.Split("(")(0)

        Dim mRemovedCompListForDue As RemovedCompListForCombo
        mRemovedCompListForDue = RemovedCompListForCombo.GetCompList(AsOnDate.ToString, PartName.Trim, prefixText)

        If count = 0 Then
            Return (From c As RemovedCompListForCombo.RemovedCompListForComboInfo In mRemovedCompListForDue
              Select AjaxControlToolkit.AutoCompleteExtender.CreateAutoCompleteItem(c.SerialNo, c.CompID.ToString())).ToArray
        Else
            Return (From c As RemovedCompListForCombo.RemovedCompListForComboInfo In mRemovedCompListForDue
                   Select AjaxControlToolkit.AutoCompleteExtender.CreateAutoCompleteItem(c.SerialNo, c.CompID.ToString())).Take(count).ToArray
        End If
    End Function
#End Region
