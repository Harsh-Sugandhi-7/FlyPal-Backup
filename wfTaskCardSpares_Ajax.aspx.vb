...
'AJAX Conversion By : Saylee on 15-Sep-2014

Imports System.Linq
Imports System.Linq.Enumerable
Imports System.Collections.Generic

Public Class wfTaskCardSpares_Ajax
...
#Region " Business Methods "
    ...
    End Sub
    Private Sub SetPage()
        If Session("SpareEdit") Then
            lblTitle.Text = "Spare [" & mTaskCard.TaskCardSpares.CurrentItem.PartNo & " ]"
        Else
            lblTitle.Text = "Spare [New]"
        End If
        txtTaskCardNo.Text = mTaskCard.TaskCardNo
    End Sub
    Private Function setObject() As Boolean
        'mTaskCard.TaskCardSpares.CurrentItem.ItemID = New Guid(PartID) 'New Guid(cmbItemList.SelectedValue.ToString)
        ItemID = IIf(txtItemList.Text = "", Guid.Empty, mItemListForCombo(PartNo).ID)
        mTaskCard.TaskCardSpares.CurrentItem.ItemID = ItemID
        mTaskCard.TaskCardSpares.CurrentItem.PartNo = Trim(txtPartNo.Text)
        mTaskCard.TaskCardSpares.CurrentItem.Description = Trim(txtDescription.Text)
        mTaskCard.TaskCardSpares.CurrentItem.RequiredQty = Val(txtReqQty.Text)
        mTaskCard.TaskCardSpares.CurrentItem.Remark = Trim(txtRemark.Text)
        'Added By Shweta On 5-Sep-2013 For BA04092013
        mTaskCard.TaskCardSpares.CurrentItem.OffSerialNo = Trim(txtOffSerialNo.Text)
        mTaskCard.TaskCardSpares.CurrentItem.OnSerialNo = Trim(txtOnSerialNo.Text)
        'End
        Return True
    End Function
    ...
    Private Sub SaveTaskCard()
        Try
            mTaskCard.Save()
            MarkLog(Util.Action.Save, "TaskCard", "Task Card Spare : " + Chr(13) + "Part No. : " + txtPartNo.Text + "," + "Description : " + txtDescription.Text + "," + "Qty. : " + txtReqQty.Text, Util.ErrorType.NoError, mTaskCard.ID, EventLogID)
...
#Region " Data Binding "
    Private Sub DataFieldBind()
        ...
        DataBind()
        txtItemList.Text = IIf(Not mTaskCard.TaskCardSpares.CurrentItem.ItemID.Equals(Guid.Empty), mTaskCard.TaskCardSpares.CurrentItem.PartNo + " [" + mTaskCard.TaskCardSpares.CurrentItem.Description + "]", "")
        hdnpartId.Value = IIf(Not mTaskCard.TaskCardSpares.CurrentItem.ItemID.Equals(Guid.Empty), mTaskCard.TaskCardSpares.CurrentItem.ItemID.ToString, "{00000000-0000-0000-0000-000000000000}")
    End Sub
    ...
    Private Function CustomValidate1() As Boolean
        ...
        If Not mTaskCard.TaskCardSpares.IsValid Then
...
#Region " Events "
    ...
    Private Sub btnOK_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnOK.Click
        ...
        SetPartID()
        If Session("SpareEdit") = False Then
            If Trim(txtPartNo.Text) <> "" And Not mTaskCard.TaskCardSpares.Contains(mTaskCard.TaskCardSpares.CurrentItem.ID, mTaskCard.ID, Trim(txtPartNo.Text)) Then
                setObject()
            ...
            ElseIf Trim(txtPartNo.Text) <> "" And mTaskCard.TaskCardSpares.Contains(mTaskCard.TaskCardSpares.CurrentItem.ID, mTaskCard.ID, Trim(txtPartNo.Text)) Then
                MSGBoxCtrl.show(MSGBox.Message_title.Duplicate, MSGBox.Message_text.Duplicate, "", MsgBoxStyle.OkOnly, "")
                mTaskCard.TaskCardSpares.CurrentItem.Description = ""
                Exit Sub
            Else
                setObject()
                If Not CustomValidate1() Then
        ...
        Else
            ...
            If mTaskCard.TaskCardSpares.CurrentItem.IsDirty Then
                'If Not mTaskCard.TaskCardSpares.Contains(mTaskCard.TaskCardSpares.CurrentItem.ID, mTaskCard.TaskCardSpares.CurrentItem.TaskCardID, New Guid(PartID)) Then

                'Added by sachin on 12-09-23
                ...
                If EitherPartNoOrItemID = True Then
                    ...
                    'ItemID = IIf(txtItemList.Text = "", Guid.Empty, mItemListForCombo(PartNo).ID)
                    'If Not mTaskCard.TaskCardSpares.Contains(mTaskCard.TaskCardSpares.CurrentItem.ID, mTaskCard.TaskCardSpares.CurrentItem.TaskCardID, ItemID) Then
                    ''These 2 lines are Commented by sachin on 12-09-23
                ...
                ElseIf Trim(txtPartNo.Text) <> "" And mTaskCard.TaskCardSpares.Contains(mTaskCard.TaskCardSpares.CurrentItem.ID, mTaskCard.TaskCardSpares.CurrentItem.TaskCardID, Trim(txtPartNo.Text)) Then   'ElseIf Not mTaskCard.TaskCardSpares.Contains(New Guid(PartID)) Then
                    If Not CustomValidate1() Then
                        mTaskCard = clnTaskCard
                        Session("mTaskCard") = mTaskCard
                        Session("wfTaskCard.TaskCard") = clnTaskCard
    ...
    Private Sub btnClose_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnClose.Click
        If mTaskCard.TaskCardSpares.CurrentItem.IsNew And Not Session("SpareEdit") = True Then mTaskCard.TaskCardSpares.Remove(mTaskCard.TaskCardSpares.CurrentItem)
        Session("SpareEdit") = False
        upnlValidationsummary.Update()
        Session.Remove("PartID")
        Session.Remove("mItemListForCombo")
