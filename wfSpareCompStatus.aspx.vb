...
Public Class wfSpareCompStatus
...
#Region " Business Methods "
    ...
    Private Sub GetAttachment()
        If mCompStatus.IsAttachmentAdded And mFileAttach Is Nothing Then
            mFileAttach = FileAttach.GetAttachment(mCompStatus.ID)
            Session("mFileAttach") = mFileAttach
        End If
    End Sub
        ...
        If Not mFileAttach Is Nothing Then
            ...
            Else
                If (Not mCompStatus.IsNew) And IsAttachmentDeleted Then
                    FileAttach.DeleteAttachment(mFileAttach.ID, mCompStatus.ID)
                End If
    ...
    Private Sub NewRecord()
        ...
        '*********************
        mCompStatus = CompStatus.NewCompStatus(Guid.NewGuid, mCompStatus.AssemblyID, "", mHourType)
        Session("mCompStatus") = mCompStatus
    End Sub
            ...
            Select Case Result1
                Case MsgBoxResult.Yes
                    If MSGBoxCtrl.Sender = "ReqServ" Then
                        ...
                        Session("CompInstTabIndex") = tabContainer.ActiveTabIndex
                    ...
                    ElseIf MSGBoxCtrl.Sender = "DeleteService" Then
                        ...
                        Catch ex As SqlException
                            ...
                            ElseIf ex.Number = 547 Then
                                ...
                                MachineDetail = mRegno & " Part Info : " & mCompStatus.PartName + " " + mCompStatus.Description + " " + mCompStatus.SerialNo & " Monitor Info : " & Mtype
    ...
    Private Sub SetObject()
        ...
        If Not mFileAttach Is Nothing Then
            If mFileAttach.Size > 0 Then
                mCompStatus.IsAttachmentAdded = True
            Else
                mCompStatus.IsAttachmentAdded = False
            End If
        End If
    ...
    Public Sub SetGridObject()
        Dim txtCurrentCompValue As TextBox
    ...
    Private Sub AddSelectedPeroids()
        ...
        For Each mSelectPeriod In mSelectPeriods
            If mSelectPeriod.IsSelected Then
                mCompStatus.CompStatusPeriods.Add(CompStatusPeriod.NewChildSpareCompStatusPeriod(mCompStatus.ID, mCompStatus.AsOnDate.ToString, mSelectPeriod.PeriodID, True))
            End If
        Next
        Session("mCompStatus") = mCompStatus
        Session.Remove("mSelectPeriods")
    ...
    Private Sub SetPeroids()
        ...
        While i <= mPeriodlist.Count - 1
            If Not mCompStatus.CompStatusPeriods.Contains(mPeriodlist(i).ID) Then
    ...
    Private Sub SetPage()
        ...
        End If
        lblModuleTSNCaption.InnerText = "Since New Value as on " & mCompStatus.AsOnDateFormatted
    ...
    Private Sub ControlVisiblity1() 'Added By Prashant 26-Aug-2010
        If mCompStatus.CompStatusPeriods.Count > 0 Then
            ...
            ''''If mCompStatus.CompStatusPeriods.Contains(9) Or mCompStatus.CompStatusPeriods.Contains(10) Then
            ''''    lblACF.Visible = True
            ''''    txtACF.Visible = True
            ''''    lblECF.Visible = True
            ''''    txtECF.Visible = True
        ...
        'If Session("IsOpenFromMaster") = True AndmCompStatus.IsSpareComp = False Then
    ...
    Private Function Save() As Boolean
        ...
        If mCompStatus.IsValid Then
            ...
            Finally
                ...
                'Added by Saylee on 10-Feb-2020,  All27072020
                'Dim mRegNo As String = ""
                'IfmCompStatus.IsSpareComp = False Then
    ...
    Private Sub GetCompStatusForPart(ByVal PartIndex As Integer) 'Added by Saylee on 25-Aug-2009
        ...
        If mtmpCompListOnPartSelection.Count > 0 Then
            Dim tmpCompStatus As CompStatus = CompStatus.GetCompStatus(mtmpCompListOnPartSelection(0).ID, Guid.Empty, mtmpCompListOnPartSelection(0).InstalledOn.ToString)
            ...
            For Each tmpCompStatusPeriod In tmpCompStatus.CompStatusPeriods
                If Not mCompStatus.CompStatusPeriods.Contains(tmpCompStatusPeriod.PeriodID) Then
                    mCompStatus.CompStatusPeriods.Add(CompStatusPeriod.NewChildSpareCompStatusPeriod(mCompStatus.ID, mCompStatus.AsOnDate, tmpCompStatusPeriod.PeriodID, True))
...
#Region " Data Binding "
    Private Sub DataFieldBind()
        ...
        mPeriodListForCompStatus = PeriodListForCompStatus.GetPeriodListForCompStatus(Guid.Empty)
        Session("mPeriodListForCompStatus") = mPeriodListForCompStatus

        ''mCompStatus = CompStatus.GetCompStatus(mCompStatusList(index).CompStatusID,Guid.Empty, mAssemblyStatus.InstalledOn.ToString) 'AC
        ...
        If mFileAttach Is Nothing Then
            If mCompStatus.IsAttachmentAdded = True Then
                mFileAttach = FileAttach.GetAttachment(mCompStatus.ID) 'Sort = 1 - Installation
            Else
                mFileAttach = FileAttach.NewAttachment(Guid.Empty, mCompStatus.ID)
            End If
            Session("mFileAttach") = mFileAttach
        End If
        DataBind()
...
#Region " Events "
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not IsPostBack And Session("sender") = "" Then
            ...
            ' SetRights()
            ControlVisibility()
            tabContainer.ActiveTabIndex = IIf(CType(Session("CompInstTabIndex"), Integer) > 0, CType(Session("CompInstTabIndex"), Integer), 0)
        ...
        Select Case e.CommandName
            Case "DeleteRec"
                ...
                Else
                    ...
                    ControlVisiblity1() 'Added By Prashant 26-Aug-2010
                    If (Not mCompStatus.CompStatusPeriods.Contains(9) And Not mCompStatus.CompStatusPeriods.Contains(10)) Then
    ...
    Private Sub btnBack_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBack.Click
        'Added by Saylee on 10-Feb-2020,  All27072020
        'mAssemblyStatus.IsSpareAssembly

        If Not mCompStatus.IsNew Then
            Dim mCompMonitorServiceStatusList As tmpCompMonitorServiceStatusList
            mCompMonitorServiceStatusList = tmpCompMonitorServiceStatusList.GetCompMonitorServiceStatusList(mCompStatus.AsOnDate.ToString, mCompStatus.CompID, True, , , , , , , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, False, IsSpareComponent:=mCompStatus.IsSpareComp) 'ture is for mCompStatus.IsMaster 

            Dim mCompMonitorInspStatusList As tmpCompMonitorInspStatusList
            mCompMonitorInspStatusList = tmpCompMonitorInspStatusList.GetCompMonitorInspStatusList(mCompStatus.AsOnDate.ToString, mCompStatus.CompID, True, , , , , , , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, False, IsSpareComponent:=mCompStatus.IsSpareComp)
        ...
        If Session("IsOpenFromMaster") = True Then
            ...
            If mCompStatus.IsSpareComp = True Then
                Response.Redirect("wfSpareCompStatus.aspx?BackPage=" & Request.QueryString("BackPage") & "&ChildPage=" & Request.QueryString("ChildPage") & "&GChildPage=" & Request.QueryString("GChildPage") & "&GChildPage1=" & Request.QueryString("GChildPage1") & "&GChildPage2=" & Request.QueryString("GChildPage2"))
            Else
                Response.Redirect("wfSpareCompStatus.aspx?BackPage=" & Request.QueryString("BackPage") & "&ChildPage=" & Request.QueryString("ChildPage") & "&GChildPage=" & Request.QueryString("GChildPage") & "&GChildPage1=" & Request.QueryString("GChildPage1") & "&GChildPage2=" & Request.QueryString("GChildPage2"))
            End If
...
#Region " Event "
    ...
    '        ReportDetails.Add(New rptStatus(, 0, "Part and Serial No. of Component", "Code", _
    '              Me.mCompStatus.Comp.Code, , , , , , , , , , , , , , , , , _
    '              cmbInstallationStatus.SelectedItem.Text + " Value as on " & mCompStatus.AsOnDateFormatted, _
    '              dgCurrentCompValue.Columns.Item(1).HeaderText, dgCurrentCompValue.Columns.Item(2).HeaderText, _
    ...
    '        If I = 0 Then
    '            If I < RHCount Then
    '                ReportDetails.Add(New rptStatus(, 0, "Part and Serial No. of Component", "ATA Chapter", _
    '                            mCompStatus.ATAChapter, , , , , , , , , , , , , , , , , _
    '                            cmbInstallationStatus.SelectedItem.Text + " Value as on " & mCompStatus.AsOnDateFormatted, _
    '                            CType(Me.mCompStatus.CompStatusPeriods(I).PeriodName, String), _
    '                            CType(Me.mCompStatus.CompStatusPeriods(I).CompCurrentValueFormatted, String), _
    ...
    '        ElseIf I = 1 Then
    '            If I < RHCount Then
    '                ReportDetails.Add(New rptStatus(, 0, "Part and Serial No. of Component", "Part No.", _
    '                            PartNumber, , , , , , , , , , , , , , , , , _
    '                            cmbInstallationStatus.SelectedItem.Text + " Value as on " & mCompStatus.AsOnDateFormatted, _
    '                            CType(Me.mCompStatus.CompStatusPeriods(I).PeriodName, String), _
    '                            CType(Me.mCompStatus.CompStatusPeriods(I).CompCurrentValueFormatted, String), _
    ...
    '        ElseIf I = 2 Then
    '            If I < RHCount Then
    '                ReportDetails.Add(New rptStatus(, 0, "Part and Serial No. of Component", "Description", _
    '                                Me.mCompStatus.Comp.Description, , , , , , , , , , , , , , , , , _
    '                                cmbInstallationStatus.SelectedItem.Text + " Value as on " & mCompStatus.AsOnDateFormatted, _
    '                        CType(Me.mCompStatus.CompStatusPeriods(I).PeriodName, String), _
    '                        CType(Me.mCompStatus.CompStatusPeriods(I).CompCurrentValueFormatted, String), _
    '                        , CType(Me.mCompStatus.CompStatusPeriods(I).AssemblyCurrentValueFormatted, String)))
    ...
    '        ElseIf I = 3 Then
    '            If I < RHCount Then
    '                ReportDetails.Add(New rptStatus(, 0, "Part and Serial No. of Component", "Serial No.", _
    '                                Me.mCompStatus.Comp.SerialNo, , , , , , , , , , , , , , , , , _
    '                                cmbInstallationStatus.SelectedItem.Text + " Value as on " & mCompStatus.AsOnDateFormatted, _
    '                       CType(Me.mCompStatus.CompStatusPeriods(I).PeriodName, String), _
    '                        CType(Me.mCompStatus.CompStatusPeriods(I).CompCurrentValueFormatted, String), _
    '                        , CType(Me.mCompStatus.CompStatusPeriods(I).AssemblyCurrentValueFormatted, String)))
    ...
    '        ElseIf I = 4 Then
    '            If I < RHCount Then
    '                ReportDetails.Add(New rptStatus(, 0, "Part and Serial No. of Component", "Position", _
    '                                Me.mCompStatus.Position, , , , , , , , , , , , , , , , , _
    '                                cmbInstallationStatus.SelectedItem.Text + " Value as on " & mCompStatus.AsOnDateFormatted, _
    '                        CType(Me.mCompStatus.CompStatusPeriods(I).PeriodName, String), _
    '                        CType(Me.mCompStatus.CompStatusPeriods(I).CompCurrentValueFormatted, String), _
    '                        , CType(Me.mCompStatus.CompStatusPeriods(I).AssemblyCurrentValueFormatted, String)))
    ...
    '        ElseIf I = 5 Then
    '            If I < RHCount Then
    '                ReportDetails.Add(New rptStatus(, 0, "Part and Serial No. of Component", "Manufacturer", _
    '                                IIf(cmbManufacturerList.SelectedIndex > 0, cmbManufacturerList.SelectedItem.ToString, ""), , , , , , , , , , , , , , , , , _
    '                                cmbInstallationStatus.SelectedItem.Text + " Value as on " & mCompStatus.AsOnDateFormatted, _
    '                        CType(Me.mCompStatus.CompStatusPeriods(I).PeriodName, String), _
    '                        CType(Me.mCompStatus.CompStatusPeriods(I).CompCurrentValueFormatted, String), _
    '                        , CType(Me.mCompStatus.CompStatusPeriods(I).AssemblyCurrentValueFormatted, String)))
    ...
    '        ElseIf I = 6 Then
    '            If I < RHCount Then
    '                ReportDetails.Add(New rptStatus(, 0, "Part and Serial No. of Component", "", _
    '                 "", , , , , , , , , , , , , , , , , _
    '                 cmbInstallationStatus.SelectedItem.Text + " Value as on " & mCompStatus.AsOnDateFormatted, _
    '                CType(Me.mCompStatus.CompStatusPeriods(I).PeriodName, String), _
    '                CType(Me.mCompStatus.CompStatusPeriods(I).CompCurrentValueFormatted, String), _
    ...
    '        ReportDetails.Add(New rptStatus(, 1, "Installation Information of Component", "Installed On", _
    '                                         Me.mCompStatus.InstalledOnFormatted.ToString, , , , , , , , , , , , , , , , , _
    '                                         "Values at " + cmbInstallationStatus.SelectedItem.Text, _
    '                                         dgInstallationValues.Columns.Item(1).HeaderText, _
    '                                         dgInstallationValues.Columns.Item(2).HeaderText, _
    ...
    '        If m = 0 Then
    '            If m < RHCount1 Then
    '                ReportDetails.Add(New rptStatus(, 1, "Installation Information of Component", "Installation Remark", _
    '                 Me.mCompStatus.InstallationReason, , , , , , , , , , , , , , , , , "Values at " + cmbInstallationStatus.SelectedItem.Text, _
    '                         CType(Me.mCompStatus.CompStatusPeriods(m).PeriodName, String), _
    '                         CType(Me.mCompStatus.CompStatusPeriods(m).CompInstallationValueFormatted, String), _
    ...
    '        ElseIf m = 1 Then
    '            If m < RHCount1 Then
    '                ReportDetails.Add(New rptStatus(, 1, "Installation Information of Component", "Work Order No.", _
    '                 Me.mCompStatus.InstallationWONo, , , , , , , , , , , , , , , , , "Values at " + cmbInstallationStatus.SelectedItem.Text, _
    '                         CType(Me.mCompStatus.CompStatusPeriods(m).PeriodName, String), _
    '                         CType(Me.mCompStatus.CompStatusPeriods(m).CompInstallationValueFormatted, String), _
    '                         , CType(Me.mCompStatus.CompStatusPeriods(m).AssemblyInstallationValueFormatted, String)))
    '            Else
    ...
    '        ElseIf m = 2 Then
    '            If m < RHCount1 Then
    '                ReportDetails.Add(New rptStatus(, 1, "Installation Information of Component", "Remark", _
    '                                       Me.mCompStatus.InstallationRemark, , , , , , , , , , , , , , , , , "Values at " + cmbInstallationStatus.SelectedItem.Text, _
    '                                             CType(Me.mCompStatus.CompStatusPeriods(m).PeriodName, String), _
    '                                             CType(Me.mCompStatus.CompStatusPeriods(m).CompInstallationValueFormatted, String), _
    '                                              , CType(Me.mCompStatus.CompStatusPeriods(m).AssemblyInstallationValueFormatted, String)))
    '            Else
    ...
    '        ElseIf m = 3 Then
    '            If m < RHCount1 Then
    '                ReportDetails.Add(New rptStatus(, 1, "Installation Information of Component", "Done By Agency", _
    '                                       Me.mCompStatus.InstDoneBy, , , , , , , , , , , , , , , , , "Values at " + cmbInstallationStatus.SelectedItem.Text, _
    '                                             CType(Me.mCompStatus.CompStatusPeriods(m).PeriodName, String), _
    '                                             CType(Me.mCompStatus.CompStatusPeriods(m).CompInstallationValueFormatted, String), _
    '                                              , CType(Me.mCompStatus.CompStatusPeriods(m).AssemblyInstallationValueFormatted, String)))
    '            Else
    ...
    '        ElseIf m = 4 Then
    '            If m < RHCount1 Then
    '                ReportDetails.Add(New rptStatus(, 1, "Installation Information of Component", "License No.", _
    '                                       Me.mCompStatus.AllLicenceNosWithEmpName, , , , , , , , , , , , , , , , , "Values at " + cmbInstallationStatus.SelectedItem.Text, _
    '                                             CType(Me.mCompStatus.CompStatusPeriods(m).PeriodName, String), _
    '                                             CType(Me.mCompStatus.CompStatusPeriods(m).CompInstallationValueFormatted, String), _
    '                                              , CType(Me.mCompStatus.CompStatusPeriods(m).AssemblyInstallationValueFormatted, String)))
    '            Else
    ...
    '        ElseIf m = 5 Then
    '            If m < RHCount1 Then
    '                ReportDetails.Add(New rptStatus(, 1, "Installation Information of Component", "Place", _
    '                                       Me.mCompStatus.InstPlace, , , , , , , , , , , , , , , , , "Values at " + cmbInstallationStatus.SelectedItem.Text, _
    '                                             CType(Me.mCompStatus.CompStatusPeriods(m).PeriodName, String), _
    '                                             CType(Me.mCompStatus.CompStatusPeriods(m).CompInstallationValueFormatted, String), _
    '                                              , CType(Me.mCompStatus.CompStatusPeriods(m).AssemblyInstallationValueFormatted, String)))
    '            Else
    ...
    '        ElseIf m = 6 Then
    '            If m < RHCount1 Then
    '                ReportDetails.Add(New rptStatus(, 1, "Installation Information of Component", "", _
    '                                      "", , , , , , , , , , , , , , , , , "Values at " + cmbInstallationStatus.SelectedItem.Text, _
    '                                              CType(Me.mCompStatus.CompStatusPeriods(m).PeriodName, String), _
    '                                              CType(Me.mCompStatus.CompStatusPeriods(m).CompInstallationValueFormatted, String), _
    '                                              , CType(Me.mCompStatus.CompStatusPeriods(m).AssemblyInstallationValueFormatted, String)))
    ...
    '        ReportDetails.Add(New rptStatus(, 2, "Document Details", "Source Doc.", "", , , , , , , , , , , , , , , , , "", , , "Revision No.", , ""))
    '    Else
    ...
    '        If n = 0 Then
    '            If n < RHCount2 Then
    '                ReportDetails.Add(New rptStatus(, 2, "Document Details", "Book No.", "", , , , , , , , , , , , , , , , , "", , , "Page No.", , ""))
    '            Else
...
#Region " Business methods "
    ...
    Private Sub NewRecordService()
        'Added by Saylee on 10-Feb-2020,  All27072020
        Dim mHourType As Integer = 0
        'IfmCompStatus.IsSpareComp = True Then
        ...
        mCompMonitorServiceStatus = CompMonitorServiceStatus.NewCompMonitorServiceStatus(Guid.NewGuid, mCompStatus.CompID, Guid.Empty, mCompStatus.AsOnDateFormatted.ToString, mCompStatus.Comp.PartID, Guid.Empty, mCompStatus.ID, mHourType, mCompStatus)
        Session("mCompMonitorServiceStatus") = mCompMonitorServiceStatus

        Dim mCompMonitorServiceStatusList As tmpCompMonitorServiceStatusList
        mCompMonitorServiceStatusList = tmpCompMonitorServiceStatusList.GetCompMonitorServiceStatusList(mCompStatus.AsOnDate.ToString, mCompStatus.CompID, True, , , , , , , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, IsSpareAssembly:=False, IsSpareComponent:=True)
        Session("mCompMonitorServiceStatusList") = mCompMonitorServiceStatusList
        'Changed By Utkarsh On 1-Aug-2011 For All19072011
        ...
        If mComponentMaintananceListCount Is Nothing Or mComponentMaintananceListCount.MaintenanceServiceListCount = 0 Then
            ...
            Dim mModelList As ModelList 'Added By Shital 1-Oct-2020 for SpareComp
            mModelList = ModelList.GetModelList(1, , , , ) 'Added By Shital 1-Oct-2020 for SpareComp
            mPartMonitorService = PartMonitorService.NewPartMonitorService(ID, mCompStatus.Comp.PartID, mModelList.Item(0).ID, mHourType, ID)
        ...
        ElseIf mComponentMaintananceListCount.MaintenanceServiceListCount > 0 Then
            ...
            ScriptManager.RegisterStartupScript(Me, Me.GetType(), "openTranDetail", "openledgersame('wfPartMonitorServiceList_Ajax.aspx?GChildPage4=wfSpareCompStatus.aspx&GChildPage5=wfSpareCompStatus.aspx&GChildPage2=" & Request.QueryString("GChildPage2") & "&GChildPage6=wfAssemblyStatus_Ajax.aspx');", True)
        End If
    End Sub
    Private Sub EditRecordService(ByVal mId As Guid)
        ...
        Dim mCompMonitorServiceStatus As CompMonitorServiceStatus
        mCompMonitorServiceStatus = CompMonitorServiceStatus.GetCompMonitorServiceStatus(mId, Guid.Empty, mCompStatus.ID, mCompStatus.HourType, , mCompStatus, True)
        Session("mCompMonitorServiceStatus") = mCompMonitorServiceStatus
        'Changed By Utkarsh On 1-Aug-2011 For All19072011
        MachineDetail = mRegNo & " Part Info : " & mCompStatus.PartName + " " + mCompStatus.Description + " " + mCompStatus.SerialNo & " Monitor Info : " & mCompMonitorServiceStatus.PartMonitorService.PartMonitorServiceTypeName
    ...
    Private Sub EditMasterRecordService(ByVal mMasterId As Guid, ByVal mId As Guid)
        ...
        mCompMonitorServiceStatus = CompMonitorServiceStatus.GetCompMonitorServiceStatus(mId, Guid.Empty, mCompStatus.ID, mHourType, , mCompStatus, True)
        Session("mCompMonitorServiceStatus") = mCompMonitorServiceStatus

        Session("mPartMonitorService") = mPartMonitorService
        'RemoveSession()
        'Changed By Utkarsh On 1-Aug-2011 For All19072011
        MachineDetail = mRegNo & " Part Info : " & mCompStatus.PartName + " " + mCompStatus.Description + " " + mCompStatus.SerialNo & " Monitor Info : " & mPartMonitorService.PartMonitorServiceTypeName
    ...
    Private Sub SetGridService()
        ...
        For j As Integer = 0 To dgMonitorServiceStatusList.Rows.Count - 1
            B = CType(Me.dgMonitorServiceStatusList.Rows(j).Cells(19).Text, Boolean)
            If B = False Then
                dgMonitorServiceStatusList.Rows(j).Cells(18).Enabled = False
            End If
        Next
    End Sub
    Private Sub SetPageService()
        ...
        lblInfoService.Text = "List of all the Servicings on the Component as of Date: " & mCompStatus.AsOnDateFormatted & ". All the values of all the Services will be as of Date: " & mCompStatus.AsOnDateFormatted
        lblCountService.Text = "List of Component Service Status: " & dgMonitorServiceStatusList.Rows.Count & " Record(s)"
    End Sub
...
#Region " Data Binding "
    Private Sub FindNowService()
        ...
        If IsDate(mCompStatus.AsOnDateFormatted.ToString) Then
        ...
        Select Case cmbLookInService.SelectedIndex
            Case 0, -1  'All
                mCompMonitorServiceStatusList = tmpCompMonitorServiceStatusList.GetCompMonitorServiceStatusList(TransDate, mCompStatus.CompID, True, , , , , , , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, IsSpareAssembly:=False, IsSpareComponent:=True)
            Case 1  'ATA Code
                mCompMonitorServiceStatusList = tmpCompMonitorServiceStatusList.GetCompMonitorServiceStatusList(TransDate, mCompStatus.CompID, True, Val(txtCodeService.Text), , , , , , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, IsSpareAssembly:=False, IsSpareComponent:=True)
            Case 2  'Service Type ID
                mCompMonitorServiceStatusList = tmpCompMonitorServiceStatusList.GetCompMonitorServiceStatusList(TransDate, mCompStatus.CompID, True, , , , CInt(cmbSearchForService.SelectedValue), , , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, IsSpareAssembly:=False, IsSpareComponent:=True)
            Case 3 ' Work Order No.
                mCompMonitorServiceStatusList = tmpCompMonitorServiceStatusList.GetCompMonitorServiceStatusList(TransDate, mCompStatus.CompID, True, , , , , txtForService.Text.Trim, , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, IsSpareAssembly:=False, IsSpareComponent:=True)
            Case 4  'Show In C of A
                mCompMonitorServiceStatusList = tmpCompMonitorServiceStatusList.GetCompMonitorServiceStatusList(TransDate, mCompStatus.CompID, True, , , , , , True, Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, IsSpareAssembly:=False, IsSpareComponent:=True)
        End Select
        Session("mCompMonitorServiceStatusList") = mCompMonitorServiceStatusList
    ...
    Private Sub DataFieldBindService()
        If IsDate(mCompStatus.AsOnDateFormatted.ToString) Then
            ...
            mCompMonitorServiceStatusList = tmpCompMonitorServiceStatusList.GetCompMonitorServiceStatusList(mCompStatus.AsOnDateFormatted.ToString, mCompStatus.CompID, True, , , , , , , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, IsSpareAssembly:=False, IsSpareComponent:=True) 'ture is for mCompStatus.IsMaster 
...
#Region "Events"
        ...
        Select Case e.CommandName
            Case "EditRec"
                ...
                If (User.IsInRole("MachineComponentServiceView") Or User.IsInRole("MachineComponentServiceEdit")) = False Then
                    'Changed By Utkarsh On 1-Aug-2011 For All19072011
                    MachineDetail = mRegNo & " Part Info : " & mCompStatus.PartName + " " + mCompStatus.Description + " " + mCompStatus.SerialNo & " Monitor Info : " & mCompMonitorServiceStatusList(mId).PartMonitorServiceTypeName
            ...
            Case "DeleteRec"
                ...
                If (User.IsInRole("MachineComponentServiceDelete")) = False Then
                    'Added By Utkarsh On 1-Aug-2011 For All19072011
                    MachineDetail = mRegNo & " Part Info : " & mCompStatus.PartName + " " + mCompStatus.Description + " " + mCompStatus.SerialNo & " Monitor Info : " & mCompMonitorServiceStatusList(mId).PartMonitorServiceTypeName
                    MarkLog(Util.Action.Delete, "Component Service Status", User.Identity.Name & " is not Authorized User to delete " & MachineDetail, Util.ErrorType.HandledError, Guid.Empty, EventLogID)
                    'End
                    MSGBoxCtrl.show(MSGBox.Message_title.Authorization, MSGBox.Message_text.Authorization, "", MsgBoxStyle.OkOnly, "")
                    Exit Sub
                End If
    ...
    Private Sub btnPrintService_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrintService.Click, btnPrintTopService.Click
        ...
        If temp < RHCount Then
            ReportDetails.Add(New rptStatus(, 0, "Part and Serial No. of Component", "Code", _
                  Me.mCompStatus.Comp.Code, , , , , , , , , , , , , , , , , _
                  "Since New Value as on " & mCompStatus.AsOnDateFormatted, _
                 "Period", "Component", _
                    , "Assembly"))
        Else
            ReportDetails.Add(New rptStatus(, 0, "Part and Serial No. of Component", "Code", _
        ...
        For I = 0 To TotalCount - 1
            If I = 0 Then
                If I < RHCount Then
                    ReportDetails.Add(New rptStatus(, 0, "Part and Serial No. of Component", "Part No.", _
                                Me.mCompStatus.Comp.PartName, , , , , , , , , , , , , , , , , _
                                "Since New Value as on " & mCompStatus.AsOnDateFormatted, _
                                CType(Me.mCompStatus.CompStatusPeriods(I).PeriodName, String), _
                                CType(Me.mCompStatus.CompStatusPeriods(I).CompCurrentValueFormatted, String), _
                                , CType(Me.mCompStatus.CompStatusPeriods(I).AssemblyCurrentValueFormatted, String)))
        ...
        ReportDetails.Add(New rptStatus(, 2, , _
              , , , dgMonitorServiceStatusList.Columns.Item(3).HeaderText, dgMonitorServiceStatusList.Columns.Item(11).HeaderText, dgMonitorServiceStatusList.Columns.Item(4).HeaderText, _
              dgMonitorServiceStatusList.Columns.Item(5).HeaderText, dgMonitorServiceStatusList.Columns.Item(6).HeaderText, _
              dgMonitorServiceStatusList.Columns.Item(7).HeaderText, dgMonitorServiceStatusList.Columns.Item(8).HeaderText, _
    ...
    Private Sub btnCloseService_Click(sender As Object, e As System.EventArgs) Handles btnCloseService.Click, btnCloseTopService.Click
        ...
        tabContainer_ActiveTabChanged(Nothing, Nothing)
        upnlContainer.Update()
    End Sub
    ...
    Private Sub btnSelectFile_ServerClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSelectFile.ServerClick
        If mCompStatus.IsAttachmentAdded Then
            mFileAttach = FileAttach.GetAttachment(mCompStatus.ID)
        Else
            mFileAttach = FileAttach.NewAttachment(Guid.NewGuid, mCompStatus.ID)
        End If
        Session("mFileAttach") = mFileAttach
    End Sub
...
#Region " Business Methods "
    ...
    Private Sub NewRecordInspection()
        ...
        mCompMonitorInspStatus = CompMonitorInspStatus.NewCompMonitorInspStatus(Guid.NewGuid, mCompStatus.CompID, Guid.Empty, mCompStatus.AsOnDateFormatted.ToString, mCompStatus.Comp.PartID, Guid.Empty, mCompStatus.ID, mHourType)
        ...
        If mComponentMaintananceListCount Is Nothing Or mComponentMaintananceListCount.MaintenanceInspListCount = 0 Then
            ...
            Dim mModelList As ModelList 'Added By Shital 1-Oct-2020 for SpareComp
            mModelList = ModelList.GetModelList(1, , , , ) 'Added By Shital 1-Oct-2020 for SpareComp
            mPartMonitorInsp = PartMonitorInsp.NewPartMonitorInsp(ID, mCompStatus.Comp.PartID, mModelList.Item(0).ID, mCompStatus.HourType, ID)
            Session.Remove("mPartMonitorInspList")
            Session("mPartMonitorInsp") = mPartMonitorInsp
            Session("IsSpareComp") = True  'Added By Shital 1-Oct-2020 for SpareComp
        ...
        ElseIf mComponentMaintananceListCount.MaintenanceInspListCount > 0 Then
            ...
            ScriptManager.RegisterStartupScript(Me, Me.GetType(), "openTranDetail", "openledgersame('wfPartMonitorInspList_Ajax.aspx?GChildPage4=wfSpareCompStatus.aspx&GChildPage5=wfSpareCompStatus.aspx&GChildPage2=" & Request.QueryString("GChildPage2") & "&GChildPage6=wfAssemblyStatus_Ajax.aspx');", True)
        End If
    ...
    Private Sub EditRecordInspection(ByVal mId As Guid)
        ...
        Dim mCompMonitorInspStatus As CompMonitorInspStatus
        mCompMonitorInspStatus = CompMonitorInspStatus.GetCompMonitorInspStatus(mId, Guid.Empty, mCompStatus.ID, mCompStatus.HourType, IsForSpareComp:=mCompStatus.IsSpareComp)
        Session("mCompMonitorInspStatus") = mCompMonitorInspStatus
        'Changed By Utkarsh On 1-Aug-2011 For All19072011
        MachineDetail = mRegNo & " Part Info : " & mCompStatus.PartName + " " + mCompStatus.Description + " " + mCompStatus.SerialNo & " Monitor Info : " & mCompMonitorInspStatus.PartMonitorInsp.PartMonitorInspTypeName
    ...
    Private Sub EditMasterRecordInspection(ByVal mMasterId As Guid, ByVal mId As Guid)
        ...
        '*********************
        Dim mPartMonitorInsp As PartMonitorInsp
        mPartMonitorInsp = PartMonitorInsp.GetPartMonitorInsp(mMasterId, mCompStatus.HourType)
        mCompMonitorInspStatus = CompMonitorInspStatus.GetCompMonitorInspStatus(mId, Guid.Empty, mCompStatus.ID, mCompStatus.HourType, IsForSpareComp:=mCompStatus.IsSpareComp)
        Session("mCompMonitorInspStatus") = mCompMonitorInspStatus
        Session("mMachine") = mMachine
        Session("mPartMonitorInsp") = mPartMonitorInsp
        'RemoveSession()
        'Changed By Utkarsh On 1-Aug-2011 For All19072011
        MachineDetail = mRegNo & " Part Info : " & mCompStatus.PartName + " " + mCompStatus.Description + " " + mCompStatus.SerialNo & " Monitor Info : " & mPartMonitorInsp.PartMonitorInspTypeName
...
#Region " Data Binding "
    Private Sub FindNowInspection()
        ...
        Select Case cmbLookInInspection.SelectedIndex
            Case 0, -1  'All
                mCompMonitorInspStatusList = tmpCompMonitorInspStatusList.GetCompMonitorInspStatusList(TransDate, mCompStatus.CompID, True, , , , , , , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, IsSpareAssembly:=False, IsSpareComponent:=True)
            Case 1  'ATA Code
                mCompMonitorInspStatusList = tmpCompMonitorInspStatusList.GetCompMonitorInspStatusList(TransDate, mCompStatus.CompID, True, Val(txtCodeInspection.Text), , , , , , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, IsSpareAssembly:=False, IsSpareComponent:=True)
            Case 2  'Insp Type ID
                mCompMonitorInspStatusList = tmpCompMonitorInspStatusList.GetCompMonitorInspStatusList(TransDate, mCompStatus.CompID, True, , , , CInt(cmbSearchForInspection.SelectedValue), , , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, IsSpareAssembly:=False, IsSpareComponent:=True)
            Case 3 ' Work Order No.
                mCompMonitorInspStatusList = tmpCompMonitorInspStatusList.GetCompMonitorInspStatusList(TransDate, mCompStatus.CompID, True, , , , , txtForInspection.Text.Trim, , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, IsSpareAssembly:=False, IsSpareComponent:=True)
            Case 4  'Show In C of A
                mCompMonitorInspStatusList = tmpCompMonitorInspStatusList.GetCompMonitorInspStatusList(TransDate, mCompStatus.CompID, True, , , , , , True, Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, IsSpareAssembly:=False, IsSpareComponent:=True)
        End Select
        Session("mCompMonitorInspStatusList") = mCompMonitorInspStatusList
    ...
    Private Sub DataFieldBindInspection()
        If IsDate(mCompStatus.AsOnDateFormatted.ToString) Then
            ...
            mCompMonitorInspStatusList = tmpCompMonitorInspStatusList.GetCompMonitorInspStatusList(mCompStatus.AsOnDate.ToString, mCompStatus.CompID, True, , , , , , , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, IsSpareAssembly:=False, IsSpareComponent:=True)
        ...
        Select Case e.CommandName
            ...
            Case "EditRec"
                ...
                If (User.IsInRole("MachineComponentInspectionView") Or User.IsInRole("MachineComponentInspectionEdit")) = False Then
                    'Changed By Utkarsh On 1-Aug-2011 For All19072011
                    MachineDetail = mRegNo & " Part Info : " & mCompStatus.PartName + " " + mCompStatus.Description + " " + mCompStatus.SerialNo & " Monitor Info : " & mCompMonitorInspStatusList(mId).MonitorType
...
#Region " Event "
    Private Sub btnPrintInspection_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrintInspection.Click, btnPrintTopInspection.Click
        ...
        ReportDetails.Add(New rptStatus(, 2, , , , , dgMonitorInspStatusList.Columns.Item(3).HeaderText, dgMonitorInspStatusList.Columns.Item(11).HeaderText, dgMonitorInspStatusList.Columns.Item(4).HeaderText, _
              dgMonitorInspStatusList.Columns.Item(5).HeaderText, dgMonitorInspStatusList.Columns.Item(6).HeaderText, _
             dgMonitorInspStatusList.Columns.Item(7).HeaderText, dgMonitorInspStatusList.Columns.Item(8).HeaderText, _
...
#Region " Business Methods "
    ...
    Private Sub NewRecordModification()
        ...
        'mHourType = mMachine.HourType 'Commented By Prashant 1-Oct-2020 for SpareComp
        'mCompStatus.AsOnDateFormatted Added By Prashant 1-Oct-2020 for SpareComp
        '*********************
        Dim mCompMonitorModStatus As CompMonitorModStatus
        mCompMonitorModStatus = CompMonitorModStatus.NewCompMonitorModStatus(Guid.NewGuid, mCompStatus.CompID, Guid.Empty, mCompStatus.AsOnDateFormatted, _
                                                                             mCompStatus.Comp.PartID, Guid.Empty, mCompStatus.ID, mHourType)
        ...
        If mComponentMaintananceListCount Is Nothing Or mComponentMaintananceListCount.MaintenanceModListCount = 0 Then
            ...
            Dim mModelList As ModelList 'Added By Prashant 1-Oct-2020 for SpareComp
            mModelList = ModelList.GetModelList(1, , , , ) 'Added By Prashant 1-Oct-2020 for SpareComp
            mPartMonitorMod = PartMonitorMod.NewPartMonitorMod(Guid.NewGuid, mCompStatus.Comp.PartID, mModelList.Item(0).ID, mHourType)
            Session.Remove("mPartMonitorModList")
            Session("mPartMonitorMod") = mPartMonitorMod
            Session("IsSpareComp") = True  'Added By Prashant 1-Oct-2020 for SpareComp
        ...
        ElseIf mComponentMaintananceListCount.MaintenanceModListCount > 0 Then
            ...
            Session("IsSpareComp") = True  'Added By Prashant 1-Oct-2020 for SpareComp
            ScriptManager.RegisterStartupScript(Me, Me.GetType(), "openTranDetail", "openledgersame('wfPartMonitorModList_Ajax.aspx?GChildPage4=wfSpareCompStatus.aspx&GChildPage5=wfSpareCompStatus.aspx&GChildPage2=" & Request.QueryString("GChildPage2") & "&GChildPage6=wfAssemblyStatus_Ajax.aspx');", True)
        End If
    ...
    Private Sub EditRecordModification(ByVal mId As Guid)
        ...
        Dim mCompMonitorModStatus As CompMonitorModStatus
        mCompMonitorModStatus = CompMonitorModStatus.GetCompMonitorModStatus(mId, Guid.Empty, mCompStatus.ID, mCompStatus.HourType)
        Session("mCompMonitorModStatus") = mCompMonitorModStatus
        'Changed By Utkarsh On 1-Aug-2011 For All19072011
        MachineDetail = mRegNo & " Part Info : " & mCompStatus.PartName + " " + mCompStatus.Description + " " + mCompStatus.SerialNo & " Monitor Info : " & mCompMonitorModStatus.PartMonitorMod.PartMonitorModTypeName
    ...
    Private Sub EditMasterRecordModification(ByVal mMasterId As Guid, ByVal mId As Guid)
        ...
        Dim mHourType As Integer = 0
        Dim mRegNo As String = ""
        'Commented By Prashant 1-Oct-2020 for SpareComp
        'mHourType = mMachine.HourType
        'mRegNo = "Reg No. : " & mMachine.RegNo
        'Session("mMachine") = mMachine
        'End of Commented By Prashant 1-Oct-2020 for SpareComp
        '*********************
        Dim mPartMonitorMod As PartMonitorMod
        mPartMonitorMod = PartMonitorMod.GetPartMonitorMod(mMasterId, mHourType)
        mCompMonitorModStatus = CompMonitorModStatus.GetCompMonitorModStatus(mId, Guid.Empty, mCompStatus.ID, mCompStatus.HourType)
        Session("mCompMonitorModStatus") = mCompMonitorModStatus
        'Session("mMachine") = mMachine 'Commented By Prashant 1-Oct-2020 for SpareComp 
        Session("mPartMonitorMod") = mPartMonitorMod
        'Changed By Utkarsh On 1-Aug-2011 For All19072011
        MachineDetail = mRegNo & " Part Info : " & mCompStatus.PartName + " " + mCompStatus.Description + " " + mCompStatus.SerialNo & " Monitor Info : " & mPartMonitorMod.PartMonitorModTypeName
...
#Region " Data Binding "
    Private Sub FindNowModification()
        ...
        Select Case cmbLookInModification.SelectedIndex
            Case 0, -1  'All
                mCompMonitorModStatusList = tmpCompMonitorModStatusList.GetCompMonitorModStatusList(TransDate, mCompStatus.CompID, True, , , , , , , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, "", False, True)
            Case 1  'ATA Code
                mCompMonitorModStatusList = tmpCompMonitorModStatusList.GetCompMonitorModStatusList(TransDate, mCompStatus.CompID, True, Val(txtCodeModification.Text), , , , , , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, "", False, True)
            Case 2 ' Mod No
                mCompMonitorModStatusList = tmpCompMonitorModStatusList.GetCompMonitorModStatusList(TransDate, mCompStatus.CompID, True, , , , , , , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, DirectiveNo:=txtForModification.Text.Trim, IsSpareAssembly:=False, IsSpareComponent:=True)
            Case 3  'Mod Type ID
                mCompMonitorModStatusList = tmpCompMonitorModStatusList.GetCompMonitorModStatusList(TransDate, mCompStatus.CompID, True, , , , CInt(cmbSearchForModification.SelectedValue), , , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, "", False, True)
            Case 4 ' Work Order No.
                mCompMonitorModStatusList = tmpCompMonitorModStatusList.GetCompMonitorModStatusList(TransDate, mCompStatus.CompID, True, , , , , txtForModification.Text.Trim, , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, "", False, True)
            Case 5  'Show In C of A
                mCompMonitorModStatusList = tmpCompMonitorModStatusList.GetCompMonitorModStatusList(TransDate, mCompStatus.CompID, True, , , , , , True, Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, "", False, True)
        End Select
    ...
    Private Sub DataFieldBindModification()
        If IsDate(mCompStatus.AsOnDateFormatted.ToString) Then
            mCompMonitorModStatusList = tmpCompMonitorModStatusList.GetCompMonitorModStatusList(mCompStatus.AsOnDateFormatted.ToString, mCompStatus.CompID, True, , , , , , , Guid.Empty.ToString, Guid.Empty.ToString, Guid.Empty.ToString, IsSpareAssembly:=False, IsSpareComponent:=True)
        ...
        Select Case e.CommandName
            Case "EditRec"
                ...
                If (User.IsInRole("MachineComponentModificationView") Or User.IsInRole("MachineComponentModificationEdit")) = False Then
                    'Changed By Utkarsh On 1-Aug-2011 For All19072011
                    MachineDetail = mRegNo & " Part Info : " & mCompStatus.PartName + " " + mCompStatus.Description + " " + mCompStatus.SerialNo & " Monitor Info : " & mCompMonitorModStatusList(mId).MonitorType
...
#Region " Event "
    Private Sub btnPrintModification_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrintModification.Click, btnPrintTopModification.Click
        ...
        ReportDetails.Add(New rptStatus(, 2, , _
                    , , , dgMonitorModStatusList.Columns.Item(3).HeaderText, dgMonitorModStatusList.Columns.Item(11).HeaderText, dgMonitorModStatusList.Columns.Item(4).HeaderText, _
                    dgMonitorModStatusList.Columns.Item(5).HeaderText, dgMonitorModStatusList.Columns.Item(6).HeaderText, _
                    dgMonitorModStatusList.Columns.Item(7).HeaderText, dgMonitorModStatusList.Columns.Item(8).HeaderText, _
...
#Region " Event "
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        If temp < RHCount Then
            ReportDetails.Add(New rptStatus(, 0, "Part and Serial No. of Component", "Code", _
                  Me.mCompStatus.Comp.Code, , , , , , , , , , , , , , , , , _
                 String.Empty + " Value as on " & mCompStatus.AsOnDateFormatted, _
                  dgCurrentCompValue.Columns.Item(1).HeaderText, dgCurrentCompValue.Columns.Item(2).HeaderText, _
                    , dgCurrentCompValue.Columns.Item(3).HeaderText))
        Else
            ReportDetails.Add(New rptStatus(, 0, "Part and Serial No. of Component", "Code", _
        ...
        For m = 0 To TotalCount1 - 1
            If m = 0 Then
                'If m < RHCount1 Then
                '    ReportDetails.Add(New rptStatus(, 1, "Information of Component", "Installation Remark", _
                '     Me.mCompStatus.InstallationReason, , , , , , , , , , , , , , , , , "Values at " + String.Empty, _
                '             CType(Me.mCompStatus.CompStatusPeriods(m).PeriodName, String), _
                '             CType(Me.mCompStatus.CompStatusPeriods(m).CompInstallationValueFormatted, String), _
                '             , CType(Me.mCompStatus.CompStatusPeriods(m).AssemblyInstallationValueFormatted, String)))
                'Else
            ...
            ElseIf m = 1 Then
                'If m < RHCount1 Then
                '    ReportDetails.Add(New rptStatus(, 1, "Information of Component", "Work Order No.", _
                '     Me.mCompStatus.InstallationWONo, , , , , , , , , , , , , , , , , "Values at " + String.Empty, _
                '             CType(Me.mCompStatus.CompStatusPeriods(m).PeriodName, String), _
                '             CType(Me.mCompStatus.CompStatusPeriods(m).CompInstallationValueFormatted, String), _
                '             , CType(Me.mCompStatus.CompStatusPeriods(m).AssemblyInstallationValueFormatted, String)))
                'Else
            ...
            ElseIf m = 2 Then
                If m < RHCount1 Then
                    ReportDetails.Add(New rptStatus(, 1, "Information of Component", "Remark", _
                                           Me.mCompStatus.InstallationRemark, , , , , , , , , , , , , , , , , , , , , ))
                Else
            ...
            ElseIf m = 3 Then
                If m < RHCount1 Then
                    ReportDetails.Add(New rptStatus(, 1, "Information of Component", "Done By Agency", _
                                           Me.mCompStatus.InstDoneBy, , , , , , , , , , , , , , , , , , , , ,  , ))
                Else
            ...
            ElseIf m = 4 Then
                If m < RHCount1 Then
                    ReportDetails.Add(New rptStatus(, 1, "Information of Component", "License No.", _
                                           Me.mCompStatus.AllLicenceNosWithEmpName, , , , , , , , , , , , , , , , , , , , , , ))
                Else
            ...
            ElseIf m = 5 Then
                If m < RHCount1 Then
                    ReportDetails.Add(New rptStatus(, 1, "Information of Component", "Place", _
                                           Me.mCompStatus.InstPlace, , , , , , , , , , , , , , , , , , , , , , ))
                Else
            ...
            ElseIf m = 6 Then
                If m < RHCount1 Then
                    ReportDetails.Add(New rptStatus(, 1, "Information of Component", "", _
                                          "", , , , , , , , , , , , , , , , , , , , , , ))
                Else
                    ReportDetails.Add(New rptStatus(, 1, "Information of Component", "", _
                                          "", , , , , , , , , , , , , , , , , , "", "", , ""))
        ...
        Dim Report As New ReportData(mCompanyDetail.CompanyName, mCompanyDetail.Address, _
        mCompanyDetail.Tel1, mCompanyDetail.Tel2, mCompanyDetail.Fax, mCompanyDetail.Email, _
        mCompanyDetail.WebSite, "Stock Component Status Detail Report", lblTitle.Text, "", "", "", "", AppSettings("Product Version"), AppSettings("SINote"), "", "", "", "", AppSettings("Logo"))
