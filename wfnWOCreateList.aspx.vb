...
#Region " Variable Declaration "
	Public mnWO As nWO
	Public mWOCreateList As nWOList
...
#Region " Business Methods "
	...
	Private Sub GetSession()
		...
		mWOCreateList = Session("mWOCreateList")
		...
		WOStatusID = IIf(Session("WOStatusID") Is Nothing, 1, Session("WOStatusID"))
		RegNo = Session("RegNo")
	...
	Private Sub SetSession()
		...
		Session("mWOCreateList") = mWOCreateList
	...
	Private Sub RemoveSession()
		...
		Session.Remove("mtmpTransTypeID")
	End Sub

	Private Sub ClearAll()
		' mTransTypeID = Session("mTransTypeId") 'Added by Saylee on 5-Sep-2018
		'If InStr(Session("MiddleFrame"), "wfnWOList_AJAX.aspx?TransTypeId=" & mTransTypeID) <= 0 Then
		If InStr(Session("MiddleFrame"), "wfnWOCreateList.aspx?TransTypeID=" & Request.QueryString("TransTypeId")) <= 0 Then
			RemoveSession()
			Session.Remove("mWOCreateList")
	...
	Private Sub NewRecord()
		...
		If AppSettings("ClientCode") = "LNT" Or AppSettings("ClientCode") = "SUH" Then
			mnWO.IssueTo = "INDAMER AVIATION PVT. LTD"
		ElseIf AppSettings("ClientCode") = "IND" Then
			mnWO.IssueTo = "INDAMER 145"
		End If
		mnWO.WOStartDate = mnWO.WODate
		Session("mnWO") = mnWO
		Session("mTransTypeID") = mTransTypeID 'Added by Saylee on 5-Sep-2018
	End Sub
	...
	Private Sub SetTitle()
		'Dim mWOCreateList As nWOList
		'mWOCreateList = nWOList.GetWOList()
		...
		mWOCreateList = Session("mWOCreateList")
		totcnt = mWOCreateList.TotalWOCount
		Session("totcnt") = totcnt


		If mTransTypeID = TransTypeName.ThirdParty Then
			lblTitle.Text = "List of Third Party Work Order    [Total No of Record(s):-" + totcnt.ToString() + "]"
		ElseIf mTransTypeID = TransTypeName.CAMO Then
			lblTitle.Text = "List of CAMO Work Order    [Total No of Record(s):-" + totcnt.ToString() + "]"
		End If
		upnlTitle.Update()
	...
	Private Sub setVariables()
		...
		Else
			RegNo = IIf(txtRegNo.Text.Trim = "", "", txtRegNo.Text.Trim)
			ModelName = IIf(txtModelNo.Text.Trim = "", "", txtModelNo.Text.Trim)
		End If
	...
	Private Sub SetToolTip()
		If (AppSettings("ClientCode") IsNot Nothing) AndAlso (AppSettings("ClientCode") = "TAAL" Or AppSettings("ClientCode") = "GlobalJet") Then
			...
			btnAddNew.ToolTip = "Click to Add New Engineering Order"
		...
		Else
			...
			btnAddNew.ToolTip = "Click to Add New Work Order"
	...
	Private Function IsInRole(ByVal CheckFor As Rights) As Boolean
		...
		Select Case mTransTypeID
			Case Util.Trans.WO145
				'If Condition Added by Shital on 05-Mar-2020
			...
			Case Util.Trans.WOCAMO
				...
				End If
			Case Util.Trans.OJS145
				IsInRoleString = "OJSWorkOrder"
			Case Util.Trans.OJSCAMO
				IsInRoleString = "OJSWorkOrder"
	...
	Private Sub SetControl()
		...
		Else
			txtRegNo.Text = IIf(RegNo = "", "", RegNo)
			txtModelNo.Text = IIf(ModelName = "", "", ModelName) '--Changed By Utkarsh On 17-Jan-2011
		End If
		...
		If (AppSettings("ClientCode") IsNot Nothing) AndAlso (AppSettings("ClientCode") = "TAAL" Or AppSettings("ClientCode") = "GlobalJet") Then
			...
			dgWOList.DataBind()
			If mTransTypeID = TransTypeName.ThirdParty Then
				lblResult.Text = "List of Third Party Engineering Order as per criteria :" & mWOCreateList.Count & " Record(s) found."
			ElseIf mTransTypeID = TransTypeName.CAMO Then
				lblResult.Text = "List of CAMO Engineering Order as per criteria :" & mWOCreateList.Count & " Record(s) found."
			End If
		...
		Else
			...
			'lblResult.Text = "List of Work Order as per criteria :" & mWOCreateList.Count & " Record(s) found."
			If mTransTypeID = TransTypeName.ThirdParty Then
				lblResult.Text = "List of Third Party Work Order as per criteria :" & mWOCreateList.Count & " Record(s) found."
			ElseIf mTransTypeID = TransTypeName.CAMO Then
				lblResult.Text = "List of CAMO Work Order as per criteria :" & mWOCreateList.Count & " Record(s) found."
			End If
		End If
		GridColumnsVisibility()
	End Sub
	...
	Private Sub FindNow(Optional ByVal Text As String = "", Optional ByVal No As Int32 = 0, Optional ByVal FromDate As String = "", Optional ByVal ToDate As String = "", Optional ByVal RegNo As String = "", Optional ByVal ModelName As String = "", Optional ByVal WOStatusID As Integer = 0, Optional ByVal StatusID As Integer = 0, Optional ByVal AddTopItem As String = "")
		mWOCreateList = Nothing
		dgWOList.DataSource = Nothing

		'Added by Shital on 04-Mar-2020
		If chkShowOJSWO.Checked = True Then
			mtmpTransTypeID = Session("mtmpTransTypeID")
			If mtmpTransTypeID = 88 Then
				mTransTypeID = 90
			Else
				mTransTypeID = 91
			End If
		...
		End If
		mWOCreateList = nWOList.GetWOList(Text, No, FromDate, ToDate, RegNo, ModelName, StatusID, WOStatusID, AddTopItem, , mTransTypeID)
		dgWOList.DataSource = mWOCreateList
		Session("mWOCreateList") = mWOCreateList
		totcnt = mWOCreateList.TotalWOCount
		Session("totcnt") = totcnt
	End Sub
	...
	Private Sub CallFindNow(ByVal Index As Integer)
		...
		' FindNow(WOText, CInt(Val(No)), txtFromDate.Text.ToString, txtToDate.Text.ToString, RegNo:=RegNo, ModelName:=ModelName, WOStatusID:=1, StatusID:=1)

		'If cmbStatus.SelectedIndex = 0 Then
		'    FindNow(WOText, CInt(Val(No)), txtFromDate.Text.ToString, txtToDate.Text.ToString, RegNo:=RegNo, ModelName:=ModelName, WOStatusID:=0, StatusID:=0)
		'ElseIf cmbStatus.SelectedIndex = 1 Then
		'    FindNow(WOText, CInt(Val(No)), txtFromDate.Text.ToString, txtToDate.Text.ToString, RegNo:=RegNo, ModelName:=ModelName, WOStatusID:=1, StatusID:=1)
		'Else
		'    FindNow(WOText, CInt(Val(No)), txtFromDate.Text.ToString, txtToDate.Text.ToString, RegNo:=RegNo, ModelName:=ModelName, WOStatusID:=cmbStatus.SelectedValue, StatusID:=2)
		'End If
		FindNow(WOText, CInt(Val(No)), txtFromDate.Text.ToString, txtToDate.Text.ToString, RegNo:=RegNo, ModelName:=ModelName, WOStatusID:=cmbStatus.SelectedValue, StatusID:=IIf(cmbStatus.SelectedIndex = 1, 1, 0))
	End Sub
				...
				Case MsgBoxResult.Yes
					If MSGBoxCtrl.Sender = "Delete" Then
						...
						Try
							...
							TempWOID = mnWO.ID
							mWOCreateDetail = mnWO.WONumber + " Dated : " + mnWO.WODateFormatted + " Created By : " + mnWO.WOBy + IIf(Not mnWO.MachineID.Equals(Guid.Empty), " Aircraft : " + mnWO.RegNo, "") + IIf(mnWO.ModelName <> "", " Model : " + mnWO.ModelName, "") + IIf(mnWO.SerialNo <> "", " Serial No. : " + mnWO.SerialNo, "")
							MarkLog(Util.Action.Delete, "Work Order", mWOCreateDetail, Util.ErrorType.NoError, TempWOID, EventLogID)
						...
						Catch ex As SqlException
							...
							ElseIf ex.Number = 547 Then
								MarkLog(Util.Action.Delete, "Work Order", "Can't delete : " & mWOCreateDetail & " is Currently in use", Util.ErrorType.NoError, TempWOID, EventLogID)
	...
	Private Sub GridColumnsVisibility()
		...
		'    dgWOList.Columns(15).Visible = True
		'Else
...
#Region "DataFieldBind"
	...
	Private Sub SetGrid()
		...
		For j As Integer = 0 To dgWOList.Rows.Count - 1
			...
			P = CType(Me.dgWOList.Rows(j).Cells(19).Text, Boolean)

			If mTransTypeID = Util.Trans.WO145 Then
				IsReadOnly = False
			Else
				If Me.dgWOList.Rows.Item(j).Cells(3).Text = "&nbsp;" Then
					IsReadOnly = False
				ElseIf mMachineNameValueList(Me.dgWOList.Rows.Item(j).Cells(3).Text) Is Nothing Then '    If mMachineNameValueList(Me.dgWOList.Rows.Item(j).Cells(3).Text) Is Nothing Then
					IsReadOnly = True
				Else
					IsReadOnly = mMachineNameValueList(Me.dgWOList.Rows.Item(j).Cells(3).Text).IsReadOnly 'Added by Saylee - Restrict User from using ReadOnly Aircraft
				End If
			End If

			If IsReadOnly = True Then
				dgWOList.Rows(j).Cells(18).Enabled = False
			End If

			If Me.dgWOList.Rows.Item(j).Cells(11).Text = "Open" Or Me.dgWOList.Rows.Item(j).Cells(11).Text = "Authorized" Then
				dgWOList.Rows(j).Cells(16).Enabled = Not (IsReadOnly = True)
				dgWOList.Rows(j).Cells(17).Enabled = Not (IsReadOnly = True)
			Else
				dgWOList.Rows(j).Cells(16).Enabled = False
				dgWOList.Rows(j).Cells(17).Enabled = False
			End If
...
#Region "Events"
	Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
		...
		If Not IsPostBack Then
			...
			If (mTransTypeID = 88) Or (mTransTypeID = 89) Then
				mtmpTransTypeID = mTransTypeID
				Session("mtmpTransTypeID") = mtmpTransTypeID
			End If
			Session("MiddleFrame") = "wfnWOCreateList.aspx?TransTypeID=" & mTransTypeID
			DataFieldBind()
			SetControl()
		Else
			dgWOList.DataSource = mWOCreateList
	...
	Private Sub dgWOList_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles dgWOList.RowCommand
		Select Case e.CommandName
			Case "EditRec"
				...
				'   Dim mId As Guid = mWOCreateList(Index).ID
				Dim mID As Guid = New Guid(e.CommandArgument.ToString)
				Dim mDate As String = mWOCreateList(mID).WODateFormatted
				Dim mWorkOrderNo As String = mWOCreateList(mID).WONumber
				Dim mCreatedBy As String = mWOCreateList(mID).WOBy
				Dim mRegNo As String = IIf(mWOCreateList(mID).RegNo = "", "", mWOCreateList(mID).RegNo)
				Dim mModel As String = IIf(mWOCreateList(mID).ModelName = "", "", mWOCreateList(mID).ModelName)
				Dim mSerialNo As String = mWOCreateList(mID).SerialNo
				mWOCreateDetail = mWorkOrderNo + " Dated : " + mDate + " Created By : " + mCreatedBy + IIf(mRegNo <> "", " Aircraft : " + mRegNo, "") + IIf(mModel <> "", " Model : " + mModel, "") + IIf(mSerialNo <> "", " Serial No. : " + mSerialNo, "")
				MarkLog(Util.Action.Edit, "Work Order", mWOCreateDetail, Util.ErrorType.NoError, mId, EventLogID)
				EditRecord(mId)
				Session("Edit") = True
				DataFieldBind()
				SetControl()
				...
				Else
					...
					If mFileAttach.Size > 0 Then
						...
						If File.Exists(AppSettings("DOCPath")) = False Then
							...
							MarkLog(Util.Action.View, "Work Order", Detail, Util.ErrorType.HandledError, mWOCreateList(Idx).ID, EventLogID)
						End If
					End If
				End If
		End Select
	...
	Private Sub btnFindNow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnFindNow.Click
		...
		'    dgWOList.Columns(2).HeaderText = "E.O. No."
		'    dgWOList.Columns(11).HeaderText = "E.O. Status"
		'    lblResult.Text = "List of Engineering Order as per criteria :" & mWOCreateList.Count & " Record(s) found."
		'Else
		dgWOList.Columns(2).HeaderText = "W.O. No."
		dgWOList.Columns(11).HeaderText = "W.O. Status"
		'  lblResult.Text = "List of Work Order as per criteria :" & mWOCreateList.Count & " Record(s) found"
		...
		ElseIf mTransTypeID = TransTypeName.CAMO Then
			lblResult.Text = "List of CAMO Work Order as per criteria :" & mWOCreateList.Count & " Record(s) found."
		ElseIf mTransTypeID = TransTypeName.OJS145 Then
			lblResult.Text = "List of CAMO Work Order as per criteria :" & mWOCreateList.Count & " Record(s) found."
		ElseIf mTransTypeID = TransTypeName.CAMOOJS Then
			lblResult.Text = "List of CAMO Work Order as per criteria :" & mWOCreateList.Count & " Record(s) found."
		End If
	...
	Private Sub btnAddNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAddNew.Click, btnAddNewTop.Click
		...
		'End
		NewRecord()
		MarkLog(Util.Action.[New], "Work Order", "", Util.ErrorType.NoError, mnWO.ID, EventLogID)
		Dim str As String
		'str = "<script language='javascript'>  openledgersame('wfnWODetail.aspx?BackPage=index.aspx'); </script>"
		'ClientScript.RegisterStartupScript(Me.GetType(), "OpenScript", str)
		If mTransTypeID = Trans.WO145 Then
			If cmbWOType.SelectedIndex = 0 Then
				mTransTypeID = Trans.WO145
			Else
				mTransTypeID = Trans.OJS145
			End If
		ElseIf mTransTypeID = Trans.WOCAMO Then
			If cmbWOType.SelectedIndex = 0 Then
				mTransTypeID = Trans.WOCAMO
			Else
				mTransTypeID = Trans.OJSCAMO
			End If
		End If
		...
		If mTransTypeID = 90 Then
			Session("MiddleFrame") = "wfnWOCreateList.aspx?TransTypeID=88"
		ElseIf mTransTypeID = 91 Then
			Session("MiddleFrame") = "wfnWOCreateList.aspx?TransTypeID=89"
		Else
			Session("MiddleFrame") = "wfnWOCreateList.aspx?TransTypeID=" & mTransTypeID
		End If
		...
		If cmbWOType.SelectedIndex = 0 Then
			str = "openledgersame('wfnWODetail_Ajax.aspx?BackPage=index.aspx');"
		Else
			str = "openledgersame('wfnWOPendingOJSList.aspx?BackPage=index.aspx');"
		End If
	...
	Private Sub dgWOList_Sorting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewSortEventArgs) Handles dgWOList.Sorting
		mWOCreateList.Sort(e.SortExpression, ComponentModel.ListSortDirection.Ascending)
		dgWOList.DataSource = mWOCreateList
		Session("mWOCreateList") = mWOCreateList
		dgWOList.DataBind()
		SetGrid()
	...
	Private Sub MSGBoxCtrl_UserControlButtonClicked(ByVal sender As Object, ByVal e As System.EventArgs) Handles MSGBoxCtrl.UserControlButtonClicked
		MessageBoxResult()
	End Sub
	Protected Sub cmbModel_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbAircraft.SelectedIndexChanged
		'FindNow
	...
	End Sub
	Private Sub chkShowOJSWO_CheckedChanged(sender As Object, e As System.EventArgs) Handles chkShowOJSWO.CheckedChanged
