Public Class wfSearchPartListForRCI_Ajax
    ...
    Private Sub gdvItem_RowCommand(sender As Object, e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gdvItem.RowCommand
        Select Case e.CommandName
            Case "SelectRec"
                ...
                mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.ItemID = mId
                mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.Part = mItems.Item(Index).Name  'mItems.Item(Index).Name
                mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.PartDescription = mItems.Item(Index).Description 'mItems.Item(Index).Description
                mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.IsPartFromListisSerialized = mItems.Item(Index).SerialisedStatus 'mItems.Item(Index).SerialisedStatus
                mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.BaseUnitID = mItems.Item(Index).UnitID   'Added By Prashant 13-May-2010
                mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.DisplayUnitID = mItems.Item(Index).UnitID    'Added By Prashant 16-July-2010
                mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.Location = mItems.Item(Index).Location
                '-----------------Added by Vikrant on 7.3.12 FORALL03052012-------------------------------
                'mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.DisplayQty = CDec(IIf(mItems.Item(Index).SerialisedStatus, 1, 0))  'Added By Prashant 12-May-2010
                If mReceiptCumInvoice.TransTypeID = 9 Then
                    mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.DisplayQty = mQtyRemovedFromAircraft
                    mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.IssueItemID = mItems.Item(Index).IssueItemID
                    If (mItems.Item(Index).PrimaryCategoryID = 1 And AppSettings("ClientCode") = "BA" Or AppSettings("ClientCode") = "PAS" Or AppSettings("ClientCode") = "Novo") Then
                        mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.RemovedAsReturnableFromAircraft = True
                    End If
                Else
                    mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.DisplayQty = CDec(IIf(mItems.Item(Index).SerialisedStatus, 1, 0))  'Added By Prashant 12-May-2010
                End If
                ...
                '        mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.ExpiryDate = CDate(mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.StartDate).AddMonths(mItems(Index).ExpiryMonths)
                '    End If
                'End If

                mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.PrimaryCategoryID = mItems.Item(Index).PrimaryCategoryID 'Added By Prashant 4-Jun-2014 ALL03062014

                If (AppSettings("ClientCode") = "BA" Or AppSettings("ClientCode") = "PAS" Or AppSettings("ClientCode") = "Novo" And mReceiptCumInvoice.TransTypeID = 67 And (mItems.Item(Index).PrimaryCategoryID = 1 Or mItems.Item(Index).PrimaryCategoryID = 2)) Then
                    mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.IsConsiderAsAsset = True
                End If

                mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.ItemTagID = mItems.Item(Index).ItemTagID
                mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.ItemTagName = mItems.Item(Index).ItemTagName
                mReceiptCumInvoice.ReceiptCumInvoiceItems.CurrentItem.IsAirworthinss = mItems.Item(Index).IsAirworthiCheck
                ...
                Response.Redirect("wfReceiptcumInvoiceItem_Ajax.aspx?BackPage=" & "wfReceiptCumInvoice_Ajax.aspx" & "&ChildPage1=" & "wfReceiptCumInvoice_Ajax.aspx")
        End Select
    End Sub
