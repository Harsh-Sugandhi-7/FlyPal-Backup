Public Class wfrptAssetValuation_Ajax
...
#Region " Helper Methods "
    ...
    Private Sub SetReport(Optional ByVal IsExcel As Boolean = False, Optional ByVal ByMail As Boolean = False)
        ...
        Dim rpt As AssetItemValuation 'rptAssetValuation
        Dim CategoryWise As Integer = 0
        Dim Value As String = ""
        Dim ReportName As String = ""
        SetValues()
        Dim ds As New dsAssetItemValuation 'dsValuation
        If chkForCategoryWise.Checked = True Then
            myReport = New crptAssetItemValuationCategoryWise 'crptAssetValuationCategoryWise
            CategoryWise = 1
        Else
            If AppSettings("ClientCode") = "BA" Then
                myReport = New crptAssetItemValuationForBA
            Else
                myReport = New crptAssetItemValuation 'crptAssetValuation
            End If
        End If
        ...
        If (ByMail = True And rpt.Count <= 0) Then
            SendMailFile.SendMailFile(, Thread.CurrentPrincipal.Identity.Name, "Asset Valuation", "Asset Valuation", "There is no record for this search criteria.", "", _
                Session("ToSendMailIDs"), Session("CcSendMailIDs"), "", True, Remark:=Session("SendMailRemark"), ReportGeneratedBy:=Session("ReportGenratedBy"), _
                                      SmtpHost:=mModuleList.Item("AssetValuation").SmtpHost, SmtpPort:=mModuleList.Item("AssetValuation").SmtpPort, SmtpUser:=mModuleList.Item("AssetValuation").SmtpUser, SmtpPassword:=mModuleList.Item("AssetValuation").SmtpPassword)
            Exit Sub
        End If
...
#Region " Events "
    ...
    Private Sub btnByMail_Click(sender As Object, e As System.EventArgs) Handles btnByMail.Click
        ...
        Session("UserEmailID") = mModuleList.Item("AssetValuation").SendToMailID
        Session("UserCcEmailID") = mModuleList.Item("AssetValuation").SendCCMailID
        '--------------------------
        Dim Str As String
        Str = "OpenByMaiWindow();"
