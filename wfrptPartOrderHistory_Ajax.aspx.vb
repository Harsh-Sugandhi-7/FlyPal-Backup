Public Class wfrptPartOrderHistory_Ajax
...
#Region " Variable Declaration "
    ...
    Dim mPartOrderHistorySearchingCriteria As String = String.Empty
...
#Region " Helper Methods "
    ...
    Private Sub SetValues()
        ...
        mPartOrderHistorySearchingCriteria = lblPartNo.Text + ", " + lblDesc.Text.Trim
    End Sub
    Private Sub SetReport(ByVal IsExcel As Boolean)
        ...
        'order history
        myReport = New crptOrdItemHistoryNew
        rpt = rptPartHistory.GetPartHistory(PartNo, Description, 1, "", False)
        ...
        If rpt.Count <= 0 Then
           MSGBoxCtrl.show(MSGBox.Message_title.NoRecordFound, MSGBox.Message_text.NoRecordFound, "There is no record for this search criteria", MsgBoxStyle.OkOnly, "")
            Exit Sub
        Else
            RecentMenuEvent.RecentMenuItemEvent(User.Identity.Name, 703)
        End If
        ds.Clear()
        ...
        MarkLog(Util.Action.Print, "PartOrderHistory", mPartOrderHistorySearchingCriteria, Util.ErrorType.NoError, Guid.Empty, EventLogID)
    End Sub
...
#Region " Events "
    ...
    Private Sub btnExport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExport.Click
        If IsValid Then
            ...
            da.Fill(ds, "ExcelOrderrptPartHistory", rpt)
            da.Fill(ds, "rptSearchingCriteria", objsearch)

            Dim columnToRemove1 As String() = {"PartID", "PartName", "PartDescription", "OrderFor", "Type", "ReleaseNoteNo", "ReleaseNoteDate", "SerialNo", "CureDate", "ExpiryDate", "RecQty", "IssQty", "IssueTo", "StartDate", "VendorInvoiceNo", "VendorInvoiceDate", "VendorInvoiceDetail", "CureQtrs", "CureYear", "ExpQtrs", "ExpYear", "CureQtrYear", "ExpQtrYear", "BatchNo", "Location", "TransType", "AlternateParts", "TransTypeName", "EffRate", "ATAChapter", "PartStatus", "OrderNumber", "OrderDateFormatted", "OrderType", "TransTypeID", "Applicability", "ATACode", "ATANomenclature", "OrderText", "OrderNo", "OrderAmend", "IsOverhaul", "OrderTransTypeID", "OrderDate"}
            For i As Integer = 0 To columnToRemove1.Length - 1
                If ds.Tables("ExcelOrderrptPartHistory").Columns.Contains(columnToRemove1(i)) Then
                    ds.Tables("ExcelOrderrptPartHistory").Columns.Remove(columnToRemove1(i))
                End If
            Next
            ...
            If ds.Tables("ExcelOrderrptPartHistory").Columns.Contains("PHDate") Then
                ds.Tables("ExcelOrderrptPartHistory").Columns("PHDate").ColumnName = "Date "
            End If

            If ds.Tables("ExcelOrderrptPartHistory").Columns.Contains("IdentityNo") Then
                ds.Tables("ExcelOrderrptPartHistory").Columns("IdentityNo").ColumnName = "Order No."
            End If
            If ds.Tables("ExcelOrderrptPartHistory").Columns.Contains("ToFrom") Then
                ds.Tables("ExcelOrderrptPartHistory").Columns("ToFrom").ColumnName = "Order On"
            End If

            If ds.Tables("ExcelOrderrptPartHistory").Columns.Contains("OrdQty") Then
                ds.Tables("ExcelOrderrptPartHistory").Columns("OrdQty").ColumnName = "Ord Qty."
            End If
            ...
            dsNew.Merge(ds.Tables("ExcelOrderrptPartHistory"))

            dsNew.Tables("rptSearchingCriteria").TableName = "Searching Criteria"
            dsNew.Tables("ExcelOrderrptPartHistory").TableName = "Part Order History"
			Session("ExcelFileName") = "Part Order History"
            ...
            MarkLog(Util.Action.Print, "PartOrderHistory", "Export To excel " + mPartOrderHistorySearchingCriteria, Util.ErrorType.NoError, Guid.Empty, EventLogID) 'Added by Shital on 18-Jan-2021
            'If Page.IsValid Then
            '    SetValues()
            '    SetReport(True)
        Else
