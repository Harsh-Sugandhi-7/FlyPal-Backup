...
#Region " Business Methods "
    ...
    Private Sub ControlVisibility()
        ...
        If mLog.LogDetails.Count = 0 Then
            ...
            dgLogTLPDetails.Visible = False
        Else
            lblTLPGridTitle.Visible = True
            dgLogTLPDetails.Visible = True
        End If

        'Log Details Remove Column
        ...
        '  dgLogTLPDetails.Columns(23).Visible = (mIsLastLog And mLog.LogDetails.Count > 1) Or (mLog.IsNew)
    ...
    Private Sub EditLogDetailRecord(ByVal SrNo As Integer)
        ...
        Response.Redirect("wfTLPDetail_Ajax.aspx?BackPage=wfTLP_Ajax.aspx")
        '''If Session("mTypeIDForLogEdit") = 2 Then
        '''    Response.Redirect("wfTLPDetailEdit_Ajax.aspx?BackPage=wfTLP_Ajax.aspx")
        '''Else
        '''    Response.Redirect("wfTLPDetail_Ajax.aspx?BackPage=wfTLP_Ajax.aspx")
        '''End If
    End Sub
...
#Region " Data Binding "
    Private Sub GridColumnHeadingSet()
        ...
        If (AppSettings("ClientCode") = "Heligo" Or AppSettings("ClientCode") = "UHPL") Then
            ...
            Next l
        ElseIf (AppSettings("ClientCode") = "ACI") Then
            dgEnginePeriods.Columns(11).HeaderText = "N1"
            dgEnginePeriods.Columns(12).HeaderText = "Final N1"

            dgEnginePeriods.Columns(13).HeaderText = "N2"
            dgEnginePeriods.Columns(14).HeaderText = "Final N2"

            dgEnginePeriods.Columns(23).HeaderText = "Creep%"
            dgEnginePeriods.Columns(24).HeaderText = "Final Creep%"
        End If
...
#Region "Events"
    ...
    Private Sub btnFuelOil_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnFuelOil.Click
        ...
        Response.Redirect("wfLogFuelOil_Ajax.aspx?BackPage=" & Request.QueryString("BackPage") & "&ChildPage=wfTLP_Ajax.aspx")
    End Sub
    Private Sub btnDefectActionList_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDefectActionList.Click
        ...
        Response.Redirect("wfLogDefectActionList_Ajax.aspx?BackPage=" & Request.QueryString("BackPage") & "&ChildPage=wfTLP_Ajax.aspx")
    End Sub
    ...
    Private Sub btnMaintenanceAcitvity_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnMaintenanceAcitvity.Click
        ...
        Response.Redirect("wfLogMaintenanceActivity_Ajax.aspx?BackPage=" & Request.QueryString("BackPage") & "&ChildPage=wfTLP_Ajax.aspx") 'Added By Prashant 23-Aug-2018
    End Sub
    ...
    Protected Sub btnAddPilot_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnAddPilot.Click
        ...
        '  Response.Redirect("wfPilot.aspx?BackPage=" & Request.QueryString("BackPage") & "&BackPage1=wfTLP_Ajax.aspx")
        Dim bkpage As String = CType(Request.QueryString("BackPage"), String)
        ScriptManager.RegisterStartupScript(Me, Me.GetType, "OpenPilotWindow(bkpage)", "OpenPilotWindow();", True)
    End Sub
    ...
    Protected Sub btnFlightLogClassification_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnFlightLogClassification.Click
        '' Response.Redirect("wfFlightLogClassification.aspx?BackPage=" & Request.QueryString("BackPage") & "&BackPage1=wfTLP_Ajax.aspx")
        Dim bkpage As String = CType(Request.QueryString("BackPage"), String)
        ScriptManager.RegisterStartupScript(Me, Me.GetType, "OpenFlightLogClassificationWindow(bkpage)", "OpenFlightLogClassificationWindow();", True)
    End Sub
    ...
    Private Sub btnAddRoute_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAddRoute.Click
        ...
        Dim SourcetmpID As Guid
        Dim FuelOnDeparturetmp As Decimal = 0
        If Not mLog.LogDetails.Count = 0 Then
            SourcetmpID = mLog.LogDetails(mLog.LogDetails.Count - 1).DestinationID
            FuelOnDeparturetmp = mLog.LogDetails(mLog.LogDetails.Count - 1).FuelOnArrival
        End If
        Dim mLogDetail As LogDetail
        ...
        If mLog.LogDetails.Count = 0 Then
			...
			If mLog.LastLogExist Then
				'Dim tmpPrevlog As Log = Log.GetLog(mLog.PrevLogID)
				'mLogDetail.FuelOnDeparture = tmpPrevlog.LogDetails(tmpPrevlog.LogDetails.Count - 1).FuelOnArrival
				Dim tmpPrevlog As Log = Log.GetLog(mLog.PrevJourneyLogID)
				If Not tmpPrevlog.LogDetails.Count = 0 Then mLogDetail.FuelOnDeparture = tmpPrevlog.LogDetails(tmpPrevlog.LogDetails.Count - 1).FuelOnArrival
			End If
		Else
            'Setting Source Place for the new child of the Log i.e. LogDetail from last child
            mLogDetail.SourceID = SourcetmpID 'mLog.LogDetails.CurrentItem.DestinationID
            mLogDetail.FuelOnDeparture = FuelOnDeparturetmp 'mLog.LogDetails.CurrentItem.FuelOnArrival
        End If
        Session("mLogDetail") = mLogDetail
        Session("mRegNo") = mMachine.RegNo
        Session("mLog") = mLog
        Session("mMachine") = mMachine 'Added By Saylee On 12-Feb-2014 For ALL12022014-1
        Response.Redirect("wfTLPDetail_Ajax.aspx?BackPage=wfTLP_Ajax.aspx&ChildPage=Index.aspx")
    End Sub
...
#Region " TAB's " 'Tabs' added by Saylee on 3-Apr-2023
    Private Sub tabLogDetailsContainer_ActiveTabChanged(sender As Object, e As System.EventArgs) Handles tabLogDetailsContainer.ActiveTabChanged
        Select Case tabLogDetailsContainer.ActiveTabIndex
            ...
            Case 1 'Fuel Oil
                ...
                ScriptManager.RegisterStartupScript(Me, Me.GetType, "CallFuelOil();", "CallFuelOil();", True)
            Case 2
                Session("Edit") = False
                '-------------------------------

                ScriptManager.RegisterStartupScript(Me, Me.GetType, "CallSnagReporting", "CallSnagReporting()", True)
            'Case 3
            '    'SetObject()
            '    'SetAirFrameGridObject()
            '    'SetEngineGridObject(True)
            ...
            '    ScriptManager.RegisterStartupScript(Me, Me.GetType, "CallParameterList", "CallParameterList()", True)
            Case 3
                ' wfLogFlightCrew_Ajax
                ScriptManager.RegisterStartupScript(Me, Me.GetType, "CallFlightCrewList", "CallFlightCrewList()", True)
            Case 4
                SetObject()
                ScriptManager.RegisterStartupScript(Me, Me.GetType, "CallMaintActivity", "CallMaintActivity()", True)
            Case 5
                ScriptManager.RegisterStartupScript(Me, Me.GetType, "CallParameterList", "CallParameterList()", True)
        End Select
    End Sub
